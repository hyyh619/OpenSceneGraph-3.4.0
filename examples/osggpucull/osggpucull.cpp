...
{
    if (_instances.size() <= maxNumInstancesPerCell)
        return false;
...
void InstanceCell<T>::bin()
{
    ...
    for (typename std::vector<T>::iterator titr = _instances.begin(); titr != _instances.end(); ++titr)
    {
        ...
        if (!assigned)
            instancesNotAssigned.push_back(*titr);
    }
...
void createStaticRendering(osg::Group *root, GPUCullData &gpuData, const osg::Vec2 &minArea, const osg::Vec2 &maxArea, unsigned int maxNumInstancesPerCell, float lodModifier, float densityModifier, float triangleModifier, bool exportInstanceObjects)
{
    ...
    if (exportInstanceObjects)
        osgDB::writeNodeFile(*coniferTree0.get(), "coniferTree0.osgt");
    ...
    if (exportInstanceObjects)
        osgDB::writeNodeFile(*coniferTree1.get(), "coniferTree1.osgt");
    ...
    if (exportInstanceObjects)
        osgDB::writeNodeFile(*coniferTree2.get(), "coniferTree2.osgt");
    ...
    if (exportInstanceObjects)
        osgDB::writeNodeFile(*decidousTree0.get(), "decidousTree0.osgt");
    ...
    if (exportInstanceObjects)
        osgDB::writeNodeFile(*decidousTree1.get(), "decidousTree1.osgt");
    ...
    if (exportInstanceObjects)
        osgDB::writeNodeFile(*decidousTree2.get(), "decidousTree2.osgt");
    ...
    if (exportInstanceObjects)
        osgDB::writeNodeFile(*simpleHouse0.get(), "simpleHouse0.osgt");
    ...
    if (exportInstanceObjects)
        osgDB::writeNodeFile(*simpleHouse1.get(), "simpleHouse1.osgt");
    ...
    if (exportInstanceObjects)
        osgDB::writeNodeFile(*simpleHouse2.get(), "simpleHouse2.osgt");
...
void createDynamicRendering(osg::Group *root, GPUCullData &gpuData, osg::BufferTemplate<std::vector<DynamicInstance>> *instances, const osg::Vec2 &minArea, const osg::Vec2 &maxArea, float lodModifier, float densityModifier, float triangleModifier, bool exportInstanceObjects)
{
    ...
    if (exportInstanceObjects)
        osgDB::writeNodeFile(*airplaneLod0.get(), "airplaneLod0.osgt");
    ...
    if (exportInstanceObjects)
        osgDB::writeNodeFile(*airplaneLod1.get(), "airplaneLod1.osgt");
    ...
    if (exportInstanceObjects)
        osgDB::writeNodeFile(*airplaneLod2.get(), "airplaneLod2.osgt");
