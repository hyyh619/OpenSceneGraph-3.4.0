...
int main(int argc, char **argv)
{
    ...
    while (args.read("--single-window"))
        singleWindow = true;

    while (args.read("--multi-window"))
        multiWindow = true;

    unsigned int numWidgets = 0;

    if (args.argc() >= 2)
        numWidgets = std::atoi(args[1]);

    else
        return doError("Please specify the number of Widgets to use.");

    if (numWidgets <= 0)
        return doError("Please specify one or more Widgets to use.");

    if (!singleWindow && !multiWindow)
        return doError(
