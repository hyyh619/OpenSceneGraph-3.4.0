...
bool ViewDependentShadowMap::selectActiveLights(osgUtil::CullVisitor *cv, ViewDependentData *vdd) const
{
         ...
         ++itr)
    {
        ...
        if (light && light->getLightNum() >= 0)
        {
            ...
            if (settings && settings->getLightNum() >= 0 && light->getLightNum() != settings->getLightNum())
                continue;
            ...
            {
                if ((*pll_itr)->light->getLightNum() == light->getLightNum())
                    break;
            }
...
osg::Polytope ViewDependentShadowMap::computeLightViewFrustumPolytope(Frustum &frustum, LightData &positionedLight)
{
    ...
    for (unsigned int i = 0; i < 12; ++i)
    {
        ...
        if (numActive == 1)
        {
            ...
            OSG_INFO << "Internal Edge " << i << ", corner_a=" << corner_a << ", corner_b=" << corner_b << ", face_a_active=" << face_a_active << ", face_b_active=" << face_b_active << std::endl;
    }
...
struct RenderLeafBounds
{
    ...
    void handle(const osg::Vec3d &v)
    {
        ...
        if (ls.z() < min_z)
        {
            min_z = ls.z();               /* OSG_NOTICE<<" - ";*/
        }

        if (ls.z() > max_z)
        {
            max_z = ls.z();               /* OSG_NOTICE<<" + ";*/
        }
...
#endif
        if (rli.min_x_ratio > min_x_ratio)
            min_x_ratio = rli.min_x_ratio;

        if (rli.max_x_ratio < max_x_ratio)
            max_x_ratio = rli.max_x_ratio;
