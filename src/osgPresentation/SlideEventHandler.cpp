...
SlideEventHandler* SlideEventHandler::instance()
{
    return s_seh.get();
}
...
void ActiveOperators::collect(osg::Node *incomingNode, osg::NodeVisitor::TraversalMode tm)
{
         ...
         ++itr)
    {
        ...
        if (_current.count(prev) == 0)
            _outgoing.insert(prev);
        else
            _maintained.insert(prev);
    }
         ...
         ++itr)
    {
        ...
        if (_previous.count(curr) == 0)
            _incoming.insert(curr);
    }
}
...
void apply(const osg::Matrixd &matrix, osg::Light *light)
{
    ...
    if (sum_x2_y2 < 1.0)
        direction.set(_currentX, _currentY, sqrtf(1.0 - sum_x2_y2));
    else
        direction.set(_currentX, _currentY, 0.0);
...
unsigned int SlideEventHandler::getNumSlides()
{
    if (_presentationSwitch.valid())
        return _presentationSwitch->getNumChildren();
    else
        return 0;
}
...
bool SlideEventHandler::nextSlide()
{
    ...
    if (selectSlide(_activeSlide + 1))
        return true;
    else if (_loopPresentation)
...
bool SlideEventHandler::previousSlide()
{
    ...
    if (_activeSlide > 0)
        return selectSlide(_activeSlide - 1);
    else if (_loopPresentation && _presentationSwitch.valid())
        return selectSlide(_presentationSwitch->getNumChildren() - 1);
    else
        return false;
    ...
    if (_activeSlide > 0)
        return selectSlide(_activeSlide - 1, LAST_POSITION);
    else if (_loopPresentation && _presentationSwitch.valid())
        return selectSlide(_presentationSwitch->getNumChildren() - 1, LAST_POSITION);
    else
        return false;
#endif
}
...
void SlideEventHandler::updateAlpha(bool modAlphaFunc, bool modMaterial, float x, float y)
{
    ...
    if (_presentationSwitch.valid())
        _presentationSwitch->accept(uav);
    else if (_viewer->getSceneData())
