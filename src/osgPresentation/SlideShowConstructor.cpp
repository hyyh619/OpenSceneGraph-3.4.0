...
void SlideShowConstructor::setPresentationAspectRatio(const std::string&str)
{
    if (str == "Reality Theatre")
        ...
        setPresentationAspectRatio(1280.0f / 1024.0f);
    else
    {
...
void SlideShowConstructor::createPresentation()
{
    ...
    if (_loopPresentation)
        _root->addDescription("loop");

    if (_autoSteppingActive)
        _root->addDescription("auto");
...
void SlideShowConstructor::addPropertyAnimation(PresentationContext presentationContext, PropertyAnimation *propertyAnimation)
{
    ...
    case (CURRENT_PRESENTATION):
        ...
        if (_presentationSwitch.valid())
            _presentationSwitch->addUpdateCallback(propertyAnimation);
    ...
    case (CURRENT_SLIDE):
        ...
        if (_slide.valid())
            _slide->addUpdateCallback(propertyAnimation);
...
void SlideShowConstructor::addScriptCallback(PresentationContext presentationContext, ScriptCallbackType scriptCallbackType, const std::string&name)
{
    ...
    case (CURRENT_PRESENTATION):
        ...
        if (_presentationSwitch.valid())
            addScriptToNode(scriptCallbackType, name, _presentationSwitch.get());
    ...
    case (CURRENT_SLIDE):
        ...
        if (_slide.valid())
            addScriptToNode(scriptCallbackType, name, _slide.get());
...
osg::Image* SlideShowConstructor::readImage(const std::string&filename, const ImageData&imageData)
{
    ...
    if (filenames.size() == 1)
    {
        ...
        if (image.valid())
            recordOptionsFilePath(options.get());
    }
    else
    {
    ...
    if (image.valid())
    {
        if (imageData.delayTime > 0.0)
            image->setUserValue("delay", imageData.delayTime);

        if (imageData.startTime > 0.0)
            image->setUserValue("start", imageData.startTime);

        if (imageData.stopTime > 0.0)
            image->setUserValue("stop", imageData.stopTime);
    }
    else
    {
...
void SlideShowConstructor::addGraph(const std::string&contents, const PositionData&positionData, const ImageData&imageData, const ScriptData&scriptData)
{
    ...
    if (result == 0)
    {
        ...
        OSG_NOTICE << "Error: SlideShowConstructor::addGraph() system(" << command.str() << ") failed with return " << result << std::endl;
}
...
void SlideShowConstructor::addVolume(const std::string&filename, const PositionData&in_positionData, const VolumeData&volumeData, const ScriptData&scriptData)
{
    ...
    if (!layer)
    {
        ...
        {
            if (ap.valid())
                groupPropetry->addProperty(ap.get());

            if (tp.valid())
                groupPropetry->addProperty(tp.get());

            if (sr.valid())
                groupPropetry->addProperty(sr.get());

            if (srm.valid())
                groupPropetry->addProperty(srm.get());
        }


        if (sd.valid())
            groupPropetry->addProperty(sd.get());

        if (sdm.valid())
            groupPropetry->addProperty(sdm.get());

        if (tfp.valid())
            groupPropetry->addProperty(tfp.get());

        if (etfp.valid())
            groupPropetry->addProperty(etfp.get());
...

        {
            ...
            if (sdm.valid())
                cp->addProperty(sdm.get());

            if (sr.valid())
                cp->addProperty(sr.get());

            if (srm.valid())
                cp->addProperty(srm.get());
