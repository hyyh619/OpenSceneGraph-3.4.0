...
void Timeout::broadcastEvent(osgViewer::Viewer *viewer, const osgPresentation::KeyPosition &keyPos)
{
    ...
    if (keyPos._key != 0)
        event->setKey(keyPos._key);

    if (keyPos._x != FLT_MAX)
        event->setX(keyPos._x);

    if (keyPos._y != FLT_MAX)
        event->setY(keyPos._y);
    ...
    else if (nv.getVisitorType() == osg::NodeVisitor::EVENT_VISITOR)
    {
        ...
        if (needToAction)
        {
            ...
            if (_actionKeyPos._key != 0 || _actionKeyPos._x != FLT_MAX || _actionKeyPos._y != FLT_MAX)
            {
                ...
                if (SlideEventHandler::instance())
                    SlideEventHandler::instance()->dispatchEvent(_actionKeyPos);
            }
