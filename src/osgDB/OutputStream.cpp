    ...
    :   _writeImageHint(WRITE_USE_IMAGE_HINT), _useSchemaData(false), _useRobustBinaryFormat(true)
{
    ...
    if (!options->getPluginStringData("WriteImageHint").empty())
    {
        ...
        if (hintString == "IncludeData")
            _writeImageHint = WRITE_INLINE_DATA;
        else if (hintString == "IncludeFile")
            _writeImageHint = WRITE_INLINE_FILE;
        else if (hintString == "UseExternal")
            _writeImageHint = WRITE_USE_EXTERNAL;
        else if (hintString == "WriteOut")
            _writeImageHint = WRITE_EXTERNAL_FILE;
    }
    ...
    if (isBinary())
    {
        ...
        *this << attributes;
        ...
        if (!_compressorName.empty())
            ...
            *this << std::string("0");   // No compressor
