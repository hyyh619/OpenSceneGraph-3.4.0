...
bool XmlNode::read(Input &input)
{
    ...
    while (input)
    {
        ...
        else if (input.match("</"))
        {
            ...
            if (comment == name)
            {
                OSG_INFO << "end tag is matched correctly" << std::endl;
            }
            else
            {
                OSG_NOTICE << "Error: end tag is not matched correctly" << std::endl;
            }
...
bool XmlNode::writeString(const ControlMap &controlMap, std::ostream &fout, const std::string &str) const
{
         ...
         ++itr)
    {
        ...
        if (citr != controlMap._characterToControlMap.end())
            fout << citr->second;
        else
            fout.put(c);
    }
