...
virtual PagedLODList* clone()
{
    return new SetBasedPagedLODList();
}
virtual void clear()
{
    _pagedLODs.clear();
...
void DatabasePager::DatabaseThread::run()
{
    ...
    do
    {
        ...
        if (databaseRequest.valid())
        {
            ...
            if (rr.error())
                OSG_WARN << "Error in reading file " << fileName << " : " << rr.message() << std::endl;
...
void DatabasePager::removeExpiredSubgraphs(const osg::FrameStamp &frameStamp)
{
    ...
    if (s_total_max_stage_a < time_a)
        s_total_max_stage_a = time_a;
...
FindPagedLODsVisitor&operator =(const FindPagedLODsVisitor&)
{
    return *this;
}
};
