...
static unsigned int pathHash(const std::string &s)
{
    ...
    for (std::string::const_iterator it = s.begin(), itEnd = s.end(); it != itEnd; ++it)
    {
        ...
        if (c == '\\')
            c = '/';                   // We're processing a path and don't want to be affected by differences in separators
...
inline WriteType getType(const osg::Object &obj)
{
    ...
    if (dynamic_cast<const osg::Image*>(&obj))
        return WRITE_TYPE_IMAGE;

    if (dynamic_cast<const osg::HeightField*>(&obj))
        return WRITE_TYPE_HEIGHT_FIELD;

    if (dynamic_cast<const osg::Node*>(&obj))
        return WRITE_TYPE_NODE;

    if (dynamic_cast<const osg::Shader*>(&obj))
        return WRITE_TYPE_SHADER;
...
bool ExternalFileWriter::write(const osg::Object &obj, const Options *options, std::string *out_absolutePath, std::string *out_relativePath)
{
    ...
    if (it != _objects.end())
    {
        ...
        if (out_absolutePath)
            *out_absolutePath = it->second.absolutePath;

        if (out_relativePath)
            *out_relativePath = it->second.relativePath;
    ...
    if (_keepRelativePaths && !originalFileName.empty())        // if keepRelativePaths is false, absoluteSourcePath is not used, then we can skip this part
    {
        if (isAbsolutePath(originalFileName))
            absoluteSourcePath = originalFileName;
        else
            absoluteSourcePath = concatPaths(_srcDirectory, originalFileName);
    ...
    // Fill output strings
    if (out_absolutePath)
        *out_absolutePath = absoluteDestinationPath;

    if (out_relativePath)
        *out_relativePath = relativeDestinationPath;
...
bool ExternalFileWriter::absoluteObjectPathExists(const std::string &path)
{
    ...
    for (SearchMap::iterator it = bounds.first; it != bounds.second; ++it)
    {
        ...
        if (_objects[img].absolutePath == path)
            return true;
    }
