...
bool GraphicsWindowX11::createVisualInfo()
{
    ...
    else
    {
        ...
        if (_traits->alpha)
        {
            attributes.push_back(GLX_ALPHA_SIZE); attributes.push_back(_traits->alpha);
        }

        if (_traits->stencil)
        {
            attributes.push_back(GLX_STENCIL_SIZE); attributes.push_back(_traits->stencil);
        }

        #if defined(GLX_SAMPLE_BUFFERS) && defined (GLX_SAMPLES)
        if (_traits->sampleBuffers)
        {
            attributes.push_back(GLX_SAMPLE_BUFFERS); attributes.push_back(_traits->sampleBuffers);
        }

        if (_traits->samples)
        {
            attributes.push_back(GLX_SAMPLES); attributes.push_back(_traits->samples);
        }
        #endif
        // TODO
        //  GLX_AUX_BUFFERS
    ...
    if ((atom = XInternAtom(display, "_MOTIF_WM_HINTS", 0)) != None)
    {
        if (flag)
        {
            ...
            if (_traits.valid() && !_traits->supportsResize)
                wmHints.functions = wmHints.functions | MWM_FUNC_RESIZE;
        }
        else
        {
...
void GraphicsWindowX11::init()
{
    ...
    if (_traits->alpha)
    {
        attributes.push_back(EGL_ALPHA_SIZE); attributes.push_back(_traits->alpha);
    }

    if (_traits->stencil)
    {
        attributes.push_back(EGL_STENCIL_SIZE); attributes.push_back(_traits->stencil);
    }

    if (_traits->sampleBuffers)
    {
        attributes.push_back(EGL_SAMPLE_BUFFERS); attributes.push_back(_traits->sampleBuffers);
    }

    if (_traits->samples)
    {
        attributes.push_back(EGL_SAMPLES); attributes.push_back(_traits->samples);
    }
...
bool GraphicsWindowX11::checkEvents()
{
    ...
    if (baseTime > _timeOfLastCheckEvents)
    ...
    while (XPending(display))
    {
        ...
        case FocusOut:
        {
            ...
            if (ev.type == FocusOut && ev.xfocus.mode != NotifyNormal)
                break;
        ...
        {
            if (firstEventTime == 0)
                firstEventTime = ev.xmotion.time;
...
{
    if (!(state ^ keyMapGetKey(_keyMap, key)))
        return;                                        // already pressed/released
