...
void Win32WindowingSystem::enumerateDisplayDevices(DisplayDevices&displayDevices) const
{
    for (unsigned int deviceNum = 0;; ++deviceNum)
    {
        ...
        if (!::EnumDisplayDevices(NULL, deviceNum, &displayDevice, 0))
            break;
...
bool Win32WindowingSystem::getSampleOpenGLContext(OpenGLContext&context, HDC windowHDC, int windowOriginX, int windowOriginY)
{
    ...
    if (!context.makeCurrent(windowHDC, true))
        return false;
...
void GraphicsWindowWin32::destroyWindow(bool deleteNativeWindow)
{
    ...
    if (_hwnd)
    {
        ...
        if (_ownsWindow && deleteNativeWindow)
            ::DestroyWindow(_hwnd);
                                             ...
                                             bool allowSwapExchangeARB)
{
    ...
    if (traits.alpha)
        attributes.set(WGL_ALPHA_BITS_ARB,     traits.alpha);

    if (traits.stencil)
        attributes.set(WGL_STENCIL_BITS_ARB,   traits.stencil);

    if (traits.sampleBuffers)
        attributes.set(WGL_SAMPLE_BUFFERS_ARB, traits.sampleBuffers);

    if (traits.samples)
        attributes.set(WGL_SAMPLES_ARB,        traits.samples);

    if (traits.quadBufferStereo)
        attributes.enable(WGL_STEREO_ARB);
