...
struct FindSharpEdgesFunctor
{
    ...
    void operator()(unsigned int p1, unsigned int p2, unsigned int p3)
    {
        ...
        if (checkDeviation(p1, normal))
            markProblemVertex(p1);

        if (checkDeviation(p2, normal))
            markProblemVertex(p2);

        if (checkDeviation(p3, normal))
            markProblemVertex(p3);
    }
...
public:
    ...
    void duplicateProblemVertexAll(ProblemVertex *pv)
    {
             ...
             ++titr)
        {
            ...
            if (tri->_p1 == p)
                tri->_p1 = duplicated_p;

            if (tri->_p2 == p)
                tri->_p2 = duplicated_p;

            if (tri->_p3 == p)
                tri->_p3 = duplicated_p;
        }
    }
