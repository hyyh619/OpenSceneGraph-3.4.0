    ...
    _maxDistance(-1.0), _index(index - 1), _numPoints(0)
{
    ...
    for (CandList_t::const_iterator it = cands.begin(); it != cands.end(); ++it)
    {
        ...
        if (_numPoints == MaxNumIntesections)
            break;
    }
...
{
    ++_index;
    if ((_dimensionMask&PolytopeIntersector::DimZero) == 0)
        return;

    if (_limitOneIntersection && !intersections.empty())
        return;
...
{
    ++_index;
    if ((_dimensionMask&PolytopeIntersector::DimOne) == 0)
        return;
...
void operator()(const Vec3_type v1, const Vec3_type v2, const Vec3_type v3, bool /*treatVertexDataAsTemporary*/)
{
    ...
    if (_candidates.empty() && _planes.size() < 3)
        return;
...
{
    _dimensionMask = dimensionMask;
}

void setLimitOneIntersection(bool limit)
{
