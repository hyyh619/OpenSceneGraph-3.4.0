...
void SceneView::setDefaults(unsigned int options)
{
    ...
    if ((options & CLEAR_GLOBAL_STATESET))
...
void SceneView::computeLeftEyeViewport(const osg::Viewport *viewport)
{
    ...
    if (!_viewportLeft.valid())
        _viewportLeft = new osg::Viewport;
...
void SceneView::computeRightEyeViewport(const osg::Viewport *viewport)
{
    ...
    if (!_viewportRight)
        _viewportRight = new osg::Viewport;
...
void SceneView::cull()
{
    ...
    if (_displaySettings.valid() && _displaySettings->getStereo())
    {
        ...
        {
            if (!_cullVisitorLeft.valid())
                _cullVisitorLeft = dynamic_cast<CullVisitor*>(_cullVisitor->clone());

            if (!_stateGraphLeft.valid())
                _stateGraphLeft = dynamic_cast<StateGraph*>(_stateGraph->cloneType());

            if (!_renderStageLeft.valid())
                _renderStageLeft = dynamic_cast<RenderStage*>(_renderStage->clone(osg::CopyOp::DEEP_COPY_ALL));

            if (!_cullVisitorRight.valid())
                _cullVisitorRight = dynamic_cast<CullVisitor*>(_cullVisitor->clone());

            if (!_stateGraphRight.valid())
                _stateGraphRight = dynamic_cast<StateGraph*>(_stateGraph->cloneType());
...
bool SceneView::cullStage(const osg::Matrixd &projection, const osg::Matrixd &modelview, osgUtil::CullVisitor *cullVisitor, osgUtil::StateGraph *rendergraph, osgUtil::RenderStage *renderStage, osg::Viewport *viewport)
{
    ...
    case (HEADLIGHT):
        if (_light.valid())
            ...
            OSG_WARN << "Warning: no osg::Light attached to osgUtil::SceneView to provide head light.*/" << std::endl;
    ...
    case (SKY_LIGHT):
        if (_light.valid())
            ...
            OSG_WARN << "Warning: no osg::Light attached to osgUtil::SceneView to provide sky light.*/" << std::endl;
    ...
    // requirement that it must traverse the camera's children.
    {
        ...
        if (callback)
            callback->run(_camera.get(), cullVisitor);
        else
...
void SceneView::collateReferencesToDependentCameras()
{
    ...
    if (_renderStageLeft.valid())
        _renderStageLeft->collateReferencesToDependentCameras();

    if (_renderStageRight.valid())
        _renderStageRight->collateReferencesToDependentCameras();
}
...
void SceneView::clearReferencesToDependentCameras()
{
    ...
    if (_renderStageLeft.valid())
        _renderStageLeft->clearReferencesToDependentCameras();

    if (_renderStageRight.valid())
        _renderStageRight->clearReferencesToDependentCameras();
}