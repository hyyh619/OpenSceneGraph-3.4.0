...
// first endpoint
inline Vertex_index ib() const
{
    return ib_;
}

// second endpoint
inline Vertex_index ie() const
{
    return ie_;
}

// first sorted endpoint
inline Vertex_index ibs() const
{
    return ibs_;
}

// second sorted endpoint
inline Vertex_index ies() const
{
    return ies_;
}

// get the "duplicate" flag
inline bool get_duplicate() const
{
    return duplicate_;
}

// set the "duplicate" flag
inline void set_duplicate(bool v)
{
    duplicate_ = v;
}
...
inline Vertex_index a() const
{
    return a_;
}
inline Vertex_index b() const
{
    return b_;
}
inline Vertex_index c() const
{
    return c_;
}
inline void incrementa(const int delta)
{
    a_ += delta;
}
inline void incrementb(const int delta)
{
    b_ += delta;
}
inline void incrementc(const int delta)
{
    c_ += delta;
}

inline const Edge&get_edge(int idx) const
{
    return edge_[idx];
}
inline const osg::Vec3&get_circumcircle() const
{
    return cc_;
}
...
bool Sample_point_compare(const osg::Vec3&p1, const osg::Vec3&p2)
{
    ...
    if (p1.y() != p2.y())
        return p1.y() < p2.y();                   // GWM 30.06.05 - further rule if x coords are same.
...
{       // fits the 'rubberband' around the 2D points for uses as a delaunay constraint.
    ...
    for (osg::Vec3Array::reverse_iterator rvit = points->rbegin() + 1; rvit != points->rend(); rvit++)
    {
        ...
        if ((*rvit) != *(verts->begin()))
            verts->push_back(*rvit);                              // next low x/y point is next length to be tested
                    ...
                    for (unsigned int i = 0; i < prset->getNumIndices(); i++)
                    {
                        ...
                        if (i > 0 || prset->getMode() == osg::PrimitiveSet::LINE_LOOP)
                        {
                            ...
                            for (titr = triangles.begin(); titr != triangles.end() && !edgused; ++titr)
                            {
                                ...
                                if (titr->isedge(ip1, ip2))
                                    edgused = true;

                                if (titr->isedge(ip2, ip1))
                                    edgused = true;
                                    ...
                                    if (icut > 0)
                                    {
                                        ...
                                        if (tradj)
                                        {
                                            while (tradj && !tradj->usesVertex(ip2) && trisToDelete.size() < 999)
                                            {
                                                ...
                                                if (icut == 1)
                                                {
                                                    e1 = tradj->b(); e2 = tradj->c();
                                                }                                                   // icut=1 implies vertex a is not involved
                                                else if (icut == 2)
                                                {
                                                    e1 = tradj->c(); e2 = tradj->a();
                                                }
                                                else if (icut == 3)
                                                {
                                                    e1 = tradj->a(); e2 = tradj->b();
                                                }
