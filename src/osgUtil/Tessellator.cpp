...
void Tessellator::beginTessellation()
{
    ...
    if (_tobj)
    {
        ...
        if (tessNormal.length() > 0.0)
            gluTessNormal(_tobj, tessNormal.x(), tessNormal.y(), tessNormal.z());
...
void apply_imp(ARRAY &array, TYPE initialValue)
{
    ...
    if (_f1)
        val += static_cast<TYPE>(array[_i1] * _f1);

    if (_f2)
        val += static_cast<TYPE>(array[_i2] * _f2);

    if (_f3)
        val += static_cast<TYPE>(array[_i3] * _f3);

    if (_f4)
        val += static_cast<TYPE>(array[_i4] * _f4);
...
void Tessellator::retessellatePolygons(osg::Geometry &geom)
{
    ...
    if (!vertices || vertices->empty() || geom.getPrimitiveSetList().empty())
        return;
    ...
    if (nprimsetoriginal)
        geom.removePrimitiveSet(0, nprimsetoriginal);
...
void Tessellator::addContour(GLenum mode, unsigned int first, unsigned int last, osg::Vec3Array *vertices)
{
    ...
    if (mode == osg::PrimitiveSet::QUADS)
        nperprim = 4;
...
void Tessellator::addContour(osg::PrimitiveSet *primitive, osg::Vec3Array *vertices)
{
    ...
    if (primitive->getMode() == osg::PrimitiveSet::QUADS)
        nperprim = 4;
    ...
    if (!_newVertexList.empty())
    {
             ...
             ++itr)
        {
            ...
            if (normals)
            {
                ...
                if (newVertex._v1)
                    norm += (*normals)[vertexPtrToIndexMap[newVertex._v1]] * newVertex._f1;

                if (newVertex._v2)
                    norm += (*normals)[vertexPtrToIndexMap[newVertex._v2]] * newVertex._f2;

                if (newVertex._v3)
                    norm += (*normals)[vertexPtrToIndexMap[newVertex._v3]] * newVertex._f3;

                if (newVertex._v4)
                    norm += (*normals)[vertexPtrToIndexMap[newVertex._v4]] * newVertex._f4;
                ...
                if (prim->_mode == GL_TRIANGLES)
                {
                    ...
                    if (osg::getBinding(geom.getColorArray()) == osg::Array::BIND_PER_PRIMITIVE_SET)   // need one per triangle? Not one per set.
                    {
                        for (int ii = 1; ii < ntris; ii++)
                        {
                            ...
                            {
                                if (cols3)
                                    cols3->push_back(primCol3);
                            }

                            if (cols4 && _index >= cols4->size())
                            {
                                if (cols4)
                                    cols4->push_back(primCol4);
                            }
