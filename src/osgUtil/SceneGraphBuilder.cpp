...
void SceneGraphBuilder::Vertex3f(GLfloat x, GLfloat y, GLfloat z)
{
    ...
    if (_texCoords.valid())
        _texCoords->push_back(_texCoord);
}
                             ...
                             GLint /*loops*/)
{
    ...
    if (_quadricState._normals != GLU_NONE)
    ...
    case (GLU_POINT):
    {
        ...
        {
            if (_quadricState._texture)
                TexCoord2f(GLfloat(sin(angle) * 0.5 + 0.5), GLfloat(cos(angle) * 0.5 + 0.5));
...
{
    if (_group.valid() && _group->getNumChildren() > 0)
        return _group.get();
    else if (_transform.valid() && _transform->getNumChildren() > 0)
        return _transform.get();
    else if (_geode.valid() && _geode->getNumDrawables() > 0)
        return _geode.get();
