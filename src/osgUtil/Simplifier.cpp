...
void setComputeErrorMetricUsingLength(bool flag)
{
    _computeErrorMetricUsingLength = flag;
}
bool getComputeErrorMetricUsingLength() const
{
    return _computeErrorMetricUsingLength;
}

unsigned int getNumOfTriangles()
{
    return _triangleSet.size();
}
...
struct Point : public osg::Referenced
{
    ...
    {
        if (_vertex < rhs._vertex)
            return true;

        if (rhs._vertex < _vertex)
            return false;
    ...
    bool isBoundaryPoint() const
    {
             ...
             ++itr)
        {
            ...
            if ((triangle->_e1->_p1 == this || triangle->_e1->_p2 == this) && triangle->_e1->isBoundaryEdge())
                return true;

            if ((triangle->_e2->_p1 == this || triangle->_e2->_p2 == this) && triangle->_e2->isBoundaryEdge())
                return true;

            if ((triangle->_e3->_p1 == this || triangle->_e3->_p2 == this) && triangle->_e3->isBoundaryEdge())
                return true;
...
struct Edge : public osg::Referenced
{
    ...
    void setErrorMetric(error_type errorMetric)
    {
    ...
    bool operator <(const Edge &rhs) const
    {
        ...
        if (getErrorMetric() < rhs.getErrorMetric())
            return true;
        else if (rhs.getErrorMetric() < getErrorMetric())
            return false;

        if (dereference_check_less(_p1, rhs._p1))
            return true;

        if (dereference_check_less(rhs._p1, _p1))
            return false;
    ...
    error_type getMaxNormalDeviationOnEdgeCollapse() const
    {
...
struct Triangle : public osg::Referenced
{
    ...
    inline bool operator <(const Triangle &rhs) const
    {
        ...
        if (dereference_check_less(lhs_lower, rhs_lower))
            return true;

        if (dereference_check_less(rhs_lower, lhs_lower))
            return false;
    ...
    void setOrderedPoints(Point *p1, Point *p2, Point *p3)
    {
        ...
        if (dereference_check_less(points[1], points[lowest]))
            lowest = 1;

        if (dereference_check_less(points[2], points[lowest]))
            lowest = 2;
...
Edge* replaceEdgePoint(Edge *edge, Point *pOriginal, Point *pNew)
{
    if (edge->_p1 == pOriginal || edge->_p2 == pOriginal)
    {
        ...
        // modify its values
        if (edge->_p1 == pOriginal)
            edge->_p1 = pNew;

        if (edge->_p2 == pOriginal)
            edge->_p2 = pNew;
...
bool divideEdge(Edge *edge, Point *pNew)
{
         ...
         ++titr)
    {
        ...
        if (edge->_p1 == tri->_p1)
        {
            if (edge->_p2 == tri->_p2.get())
                edgeToReplace = 1;                                  // edge p1,p2
            else if (edge->_p2 == tri->_p3.get())
                edgeToReplace = 3;                                       // edge p3, p1
        }
        else if (edge->_p1 == tri->_p2.get())
        {
            if (edge->_p2 == tri->_p3.get())
                edgeToReplace = 2;                                  // edge p2,p3
            else if (edge->_p2 == tri->_p1.get())
                edgeToReplace = 1;                                       // edge p1, p2
        }
        else if (edge->_p1 == tri->_p3.get())
        {
            if (edge->_p2 == tri->_p1.get())
                edgeToReplace = 3;                                  // edge p3,p1
            else if (edge->_p2 == tri->_p2.get())
                edgeToReplace = 2;                                       // edge p2, p3
        }
...
struct CollectTriangleOperator
{
    ...
    void setEdgeCollapse(EdgeCollapse *ec)
    {
        _ec = ec;
    }
...
CopyArrayToPointsVisitor&operator =(const CopyArrayToPointsVisitor&)
{
    return *this;
}
};
...
CopyVertexArrayToPointsVisitor&operator =(const CopyVertexArrayToPointsVisitor&)
{
    return *this;
}
};
...
virtual void apply(osg::UByteArray &array)
{
    copy(array, dummy_uchar());
}
virtual void apply(osg::UShortArray &array)
{
    copy(array, dummy_ushort());
}
virtual void apply(osg::UIntArray &array)
{
    copy(array, dummy_uint());
}
virtual void apply(osg::FloatArray &array)
{
    copy(array, float());
...
virtual void apply(osg::Vec2Array &array)
{
    normalize(array.begin(), array.end());
}
virtual void apply(osg::Vec3Array &array)
{
    normalize(array.begin(), array.end());
