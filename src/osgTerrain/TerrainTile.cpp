...
void TerrainTile::setTerrain(Terrain *ts)
{
    ...
    if (_terrain)
        _terrain->unregisterTerrainTile(this);
...
{
    if (_terrainTechnique == terrainTechnique)
        return;
    ...
    if (dirtyDelta > 0)
        ...
        setDirtyMask(NOT_DIRTY);
}

void TerrainTile::setDirtyMask(int dirtyMask)
{
    if (_dirtyMask == dirtyMask)
        return;
    ...
    if (dirtyDelta > 0)
    {
        ...
        if (_terrain)
            _terrain->updateTerrainTileOnNextFrame(this);
    }
...
{
    if (_colorLayers.size() <= i)
        _colorLayers.resize(i + 1);
...
osg::BoundingSphere TerrainTile::computeBound() const
{
    ...
    else
    {
        ...
        {
            if (itr->valid())
                bs.expandBy((*itr)->computeBound(false));
        }
    }
            ...
            for (unsigned int si = 0; si < switchLayer->getNumLayers(); ++si)
            {
                ...
                if (imageLayer)
                {
                    if (readImageLayer(imageLayer, options))
                    {
                        ...
                        if (_replaceSwitchLayer)
                            tile->setColorLayer(i, imageLayer);
                        else if (switchLayer->getActiveLayer() < 0)
                            switchLayer->setActiveLayer(si);
