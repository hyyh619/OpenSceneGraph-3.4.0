...
void GeometryTechnique::setFilterBias(float filterBias)
{
    ...
    if (!_filterBiasUniform)
        _filterBiasUniform = new osg::Uniform("filterBias", _filterBias);
    else
        _filterBiasUniform->set(filterBias);
}

void GeometryTechnique::setFilterWidth(float filterWidth)
{
    _filterWidth = filterWidth;
    if (!_filterWidthUniform)
        _filterWidthUniform = new osg::Uniform("filterWidth", _filterWidth);
    else
...
void GeometryTechnique::setFilterMatrix(const osg::Matrix3&matrix)
{
    ...
    if (!_filterMatrixUniform)
        _filterMatrixUniform = new osg::Uniform("filterMatrix", _filterMatrix);
    else
        _filterMatrixUniform->set(_filterMatrix);
}
...
osg::Vec3d GeometryTechnique::computeCenterModel(BufferData&buffer, Locator *masterLocator)
{
    ...
    if (!elevationLocator)
        elevationLocator = masterLocator;

    if (!colorLocator)
        colorLocator = masterLocator;
...
inline int&index(int c, int r)
{
    return _indices[(r + 1) * (_numColumns + 2) + c + 1];
}
...
inline int vertex_index(int c, int r) const
{
...
inline bool computeNormalWithDiagonals(int c, int r, osg::Vec3&n) const
{
    ...
    if (left_valid)
    {
        dx = center - left;
        if (top_left_valid)
            dy += (top_left - left) * ratio;

        if (bottom_left_valid)
            dy += (left - bottom_left) * ratio;
    }
    ...
    if (right_valid)
    {
        dx = right - center;
        if (top_right_valid)
            dy += (top_right - right) * ratio;

        if (bottom_right_valid)
            dy += (right - bottom_right) * ratio;
    }
    ...
    if (bottom_valid)
    {
        dy += center - bottom;
        if (bottom_left_valid)
            dx += (bottom - bottom_left) * ratio;

        if (bottom_right_valid)
            dx += (bottom_right - bottom) * ratio;
    }
    ...
    if (top_valid)
    {
        dy += top - center;
        if (top_left_valid)
            dx += (top - top_left) * ratio;

        if (top_right_valid)
            dx += (top_right - top) * ratio;
    }
    ...
    for (int j = 0; j < _numRows; ++j)
    {
        for (int i = 0; i < _numColumns; ++i)
        {
            ...
            if (elevationLayer)
            {
                float value = 0.0f;
                if (sampled)
                    validValue = elevationLayer->getInterpolatedValidValue(ndc.x(), ndc.y(), value);
                else
    ...
    for (int j = 0; j < _numRows; ++j)
    {
        for (int i = -1; i <= 0; ++i)
        {
            ...
            if (elevationLayer)
            {
                float value = 0.0f;
                if (sampled)
                    validValue = elevationLayer->getInterpolatedValidValue(left_ndc.x(), left_ndc.y(), value);
                else
                    validValue = elevationLayer->getValidValue((_numColumns - 1) + i, j, value);
    ...
    for (int j = 0; j < _numRows; ++j)
    {
        for (int i = _numColumns - 1; i < _numColumns + 1; ++i)
        {
            ...
            if (elevationLayer)
            {
                float value = 0.0f;
                if (sampled)
                    validValue = elevationLayer->getInterpolatedValidValue(right_ndc.x(), right_ndc.y(), value);
                else
                    validValue = elevationLayer->getValidValue(i - (_numColumns - 1), j, value);
    ...
    for (int j = _numRows - 1; j < _numRows + 1; ++j)
    {
        for (int i = 0; i < _numColumns; ++i)
        {
            ...
            if (elevationLayer)
            {
                float value = 0.0f;
                if (sampled)
                    validValue = elevationLayer->getInterpolatedValidValue(above_ndc.x(), above_ndc.y(), value);
                else
                    validValue = elevationLayer->getValidValue(i, j - (_numRows - 1), value);
    ...
    for (int j = -1; j <= 0; ++j)
    {
        for (int i = 0; i < _numColumns; ++i)
        {
            ...
            if (elevationLayer)
            {
                float value = 0.0f;
                if (sampled)
                    validValue = elevationLayer->getInterpolatedValidValue(below_ndc.x(), below_ndc.y(), value);
                else
                    validValue = elevationLayer->getValidValue(i, (_numRows - 1) + j, value);
    ...
    for (int j = 0; j < _numRows; ++j)
    {
        for (int i = 0; i < _numColumns; ++i)
        {
            ...
            if (vi >= 0)
                ...
                OSG_NOTICE << "Not computing normal, vi=" << vi << std::endl;
        }
    }
}
...
void GeometryTechnique::generateGeometry(BufferData&buffer, Locator *masterLocator, const osg::Vec3d&centerModel)
{
    ...
    if (terrain && terrain->getEqualizeBoundaries())
    {
        ...
        if (left_tile.valid())
            addNeighbour(left_tile.get());

        if (right_tile.valid())
            addNeighbour(right_tile.get());

        if (top_tile.valid())
            addNeighbour(top_tile.get());

        if (bottom_tile.valid())
            addNeighbour(bottom_tile.get());

#if 0
        if (bottom_left_tile.valid())
            addNeighbour(bottom_left_tile.get());

        if (bottom_right_tile.valid())
            addNeighbour(bottom_right_tile.get());

        if (top_left_tile.valid())
            addNeighbour(top_left_tile.get());

        if (top_right_tile.valid())
            addNeighbour(top_right_tile.get());
#endif
        ...
        if (right_tile.valid())
        {
            if (!(right_tile->getTerrainTechnique()->containsNeighbour(_terrainTile)))
            {
                ...
                if (updateNeighboursImmediately)
                    right_tile->init(dirtyMask, true);
                else
        ...
        if (bottom_tile.valid())
        {
            if (!(bottom_tile->getTerrainTechnique()->containsNeighbour(_terrainTile)))
            {
                ...
                if (updateNeighboursImmediately)
                    bottom_tile->init(dirtyMask, true);
                else
        ...
        if (bottom_left_tile.valid())
        {
            if (!(bottom_left_tile->getTerrainTechnique()->containsNeighbour(_terrainTile)))
            {
                ...
                if (updateNeighboursImmediately)
                    bottom_left_tile->init(dirtyMask, true);
                else
                    bottom_left_tile->setDirtyMask(dirtyMask);
            }
        }
        ...
        if (bottom_right_tile.valid())
        {
            if (!(bottom_right_tile->getTerrainTechnique()->containsNeighbour(_terrainTile)))
            {
                ...
                if (updateNeighboursImmediately)
                    bottom_right_tile->init(dirtyMask, true);
                else
                    bottom_right_tile->setDirtyMask(dirtyMask);
            }
        }
        ...
        if (top_right_tile.valid())
        {
            if (!(top_right_tile->getTerrainTechnique()->containsNeighbour(_terrainTile)))
            {
                ...
                if (updateNeighboursImmediately)
                    top_right_tile->init(dirtyMask, true);
                else
                    top_right_tile->setDirtyMask(dirtyMask);
            }
        }
        ...
        if (top_left_tile.valid())
        {
            if (!(top_left_tile->getTerrainTechnique()->containsNeighbour(_terrainTile)))
            {
                ...
                if (updateNeighboursImmediately)
                    top_left_tile->init(dirtyMask, true);
                else
                    top_left_tile->setDirtyMask(dirtyMask);
            }
        }
#endif
    }
...
{
    if (_currentBufferData.valid() && _currentBufferData->_transform.valid())
        _currentBufferData->_transform->releaseGLObjects(state);

    if (_newBufferData.valid() && _newBufferData->_transform.valid())
        _newBufferData->_transform->releaseGLObjects(state);
}