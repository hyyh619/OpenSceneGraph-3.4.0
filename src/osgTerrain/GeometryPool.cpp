...
void GeometryPool::applyLayers(osgTerrain::TerrainTile *tile, osg::StateSet *stateset)
{
    ...
    // stateset->setDefine("GL_LIGHTING", osg::StateAttribute::ON);
    {
        ...
        if (!_rootStateSetAssigned)
        {
            ...
            if (num_Color >= 2)
                _rootStateSet->setDefine("COLOR_LAYER1");

            if (num_Color >= 3)
                _rootStateSet->setDefine("COLOR_LAYER2");
...
void SharedGeometry::resizeGLObjectBuffers(unsigned int maxSize)
{
    ...
    if (vbo)
        vbo->resizeGLObjectBuffers(maxSize);

    osg::BufferObject *ebo = _drawElements->getElementBufferObject();
    if (ebo)
        ebo->resizeGLObjectBuffers(maxSize);
}
...
void SharedGeometry::drawImplementation(osg::RenderInfo &renderInfo) const
{
    ...
    if (computeDiagonals)
    {
        ...
        {}
            ...
            OSG_NOTICE << "SharedGeometry::drawImplementation. OK computeDiagonals=TRUE" << std::endl;
    }
    else
    {
        ...
        {}
            ...
            OSG_NOTICE << "SharedGeometry::drawImplementation. OK computeDiagonals=FALSE" << std::endl;
    }
#endif
