...
void _processRow(unsigned int num, GLenum pixelFormat, T *data, const O &operation)
{
    switch (pixelFormat)
    {
    case (GL_LUMINANCE):         { for (unsigned int i = 0; i < num; ++i)
                                   {
                                       operation(*data++);
                                   }
    }  break;

    case (GL_ALPHA):             { for (unsigned int i = 0; i < num; ++i)
                                   {
                                       operation(*data++);
                                   }
    }  break;

    case (GL_LUMINANCE_ALPHA):   { for (unsigned int i = 0; i < num; ++i)
                                   {
                                       operation(*data++); operation(*data++);
                                   }
    }  break;

    case (GL_RGB):               { for (unsigned int i = 0; i < num; ++i)
                                   {
                                       operation(*data++); operation(*data++); operation(*data++);
                                   }
    }  break;

    case (GL_RGBA):              { for (unsigned int i = 0; i < num; ++i)
                                   {
                                       operation(*data++); operation(*data++); operation(*data++); operation(*data++);
                                   }
    }  break;

    case (GL_BGR):               { for (unsigned int i = 0; i < num; ++i)
                                   {
                                       operation(*data++); operation(*data++); operation(*data++);
                                   }
    }  break;

    case (GL_BGRA):              { for (unsigned int i = 0; i < num; ++i)
                                   {
                                       operation(*data++); operation(*data++); operation(*data++); operation(*data++);
                                   }
    }  break;
...
{
    if (getImage())
        getImage()->setModifiedCount(value);
}
...
unsigned int ContourLayer::getModifiedCount() const
{
    if (!getImage())
        ...
        return getImage()->getModifiedCount();
}
...
{
    if (_implementation.valid())
        return _implementation->getNumColumns();
    else
        return 0;
}
...
{
    if (_implementation.valid())
        return _implementation->getNumRows();
    else
        return 0;
}
...
{
    if (_implementation.valid())
        return _implementation->transform(offset, scale);
    else
        return false;
}
...
{
    if (_implementation.valid())
        _implementation->dirty();
}

void ProxyLayer::setModifiedCount(unsigned int value)
{
    if (_implementation.valid())
        _implementation->setModifiedCount(value);
}
...
{
    if (_implementation.valid())
        return _implementation->computeBound(treatAsElevationLayer);
    else
