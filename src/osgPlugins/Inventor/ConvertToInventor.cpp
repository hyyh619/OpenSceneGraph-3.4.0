...
static void osgArray2ivMField(const osg::Array *array, SoMField&field, int startIndex = 0, int stopIndex = 0, int numItemsUntilMinusOne = 0)
{
    ...
    else if (ivApplicateIntType<SoMFInt32,  int32_t>(array, (SoMFInt32&) field, startIndex, stopIndex, numItemsUntilMinusOne))
        return;
    else if (ivApplicateIntType<SoMFUInt32, uint32_t>(array, (SoMFUInt32&)field, startIndex, stopIndex, numItemsUntilMinusOne))
        return;
    else if (ivApplicateIntType<SoMFShort,  int16_t>(array, (SoMFShort&) field, startIndex, stopIndex, numItemsUntilMinusOne))
        return;
    else if (ivApplicateIntType<SoMFUShort, uint16_t>(array, (SoMFUShort&)field, startIndex, stopIndex, numItemsUntilMinusOne))
        return;
            ...
            *(dst++) = *(src++);
            ...
            for (int j = 3; j < c; j++)
            {
                *(dst++) = -1;
                ...
                else if (primType == osg::PrimitiveSet::TRIANGLE_FAN)
                {
                    *(dst++) = first;
                    *(dst++) = *(src - 1);
...
static bool updateMode(bool&flag, const osg::StateAttribute::GLModeValue value)
{
    if (value & osg::StateAttribute::INHERIT)
        ...
        return (flag = (value & osg::StateAttribute::ON));
}
    ...
    if (ss)
    {
            ...
            ivState->osgCullFace != ivPrevState->osgCullFace)
        {
            ...
            else
            {
                ...
                if (ivState->osgCullFaceEnabled)
                {
                    ...
                    if (ivState->osgCullFace != osg::CullFace::BACK)
                        ccw = !ccw;
        ...
        if (useIvExtensions)
                 ...
                 ivState->osgBlendFunc->getDestinationRGB() != ivPrevState->osgBlendFunc->getDestinationRGB()))
            {
                ...
                int index = 0;
                if (!ivState->osgBlendFunc)
                    index |= 0;
                else
                    index = (ivState->osgBlendFunc->getDestinationRGB() == osg::BlendFunc::ONE) ? 1 : 0;
                                ...
                                SoSeparator*&indexedRoot, SoSeparator*&nonIndexedRoot)
{
    ...
    else
    {
        ...
        if (shape->isOfType(SoPointSet::getClassTypeId()))
            ((SoPointSet*)shape)->numPoints.setValue(elementsCount);
        else if (shape->isOfType(SoLineSet::getClassTypeId()))
        {
            switch (pset->getMode())
            {
            case GL_LINES:
        ...
        else if (shape->isOfType(SoTriangleStripSet::getClassTypeId()))
        {
            switch (pset->getMode())
            {
            case GL_TRIANGLES:
        ...
        else if (shape->isOfType(SoFaceSet::getClassTypeId()))
        {
            switch (pset->getMode())
            {
            case GL_QUADS:
    ...
    if (nonIndexedCoords)
    {
        ...
        if (!ok)
        {
            ...
            if (nonIndexedTexCoords)
            {
                nonIndexedTexCoords->ref(); nonIndexedTexCoords->unref();
            }

            if (nonIndexedNormals)
            {
                nonIndexedNormals->ref(); nonIndexedNormals->unref();
            }
        ...
        else
        {
            ...
            if (nonIndexedRoot == NULL)
            {
                ...
                if (ivTexture)
                    nonIndexedRoot->addChild(ivTexture);

                if (ivTexture)
                    nonIndexedRoot->addChild(createTexCoordBinding(FALSE));

                nonIndexedRoot->addChild(createMaterialBinding(g, FALSE));
                if (ivNormals)
                    nonIndexedRoot->addChild(createNormalBinding(g, FALSE));
            }

            if (nonIndexedMaterial)
                sep->addChild(nonIndexedMaterial);

            sep->addChild(nonIndexedCoords);
            if (nonIndexedNormals)
                sep->addChild(nonIndexedNormals);

            if (nonIndexedTexCoords)
                sep->addChild(nonIndexedTexCoords);
    ...
    else
    {
        ...
        if (indexedRoot == NULL)
        {
            ...
            if (ivTexture)
                indexedRoot->addChild(ivTexture);

            if (ivTexture)
                indexedRoot->addChild(createTexCoordBinding(TRUE));

            if (ivMaterial)
                indexedRoot->addChild(ivMaterial);

            indexedRoot->addChild(createMaterialBinding(g, TRUE));
            indexedRoot->addChild(ivCoords);
            if (ivNormals)
                indexedRoot->addChild(ivNormals);

            if (ivNormals)
                indexedRoot->addChild(createNormalBinding(g, TRUE));

            if (ivTexCoords)
                indexedRoot->addChild(ivTexCoords);
        }
...
void ConvertToInventor::processGeometry(const deprecated_osg::Geometry *g, InventorState *ivState)
{
    ...
    if (ivState->osgLighting || vrml1Conversion)
        ...
        if (g->getColorArray())
            if (ivStateMaterial)
                ivMaterial = ivStateMaterial->copy();
            else
                ivMaterial = new SoMaterial;

        // FIXME: check default values of SoMaterial and OSG lighting
        else if (ivStateMaterial)
            ivMaterial = ivStateMaterial;
        else
            ivMaterial = NULL;
    ...
    // FIXME: check default values of SoBaseColor and OSG pre-lit scene
    else if (ivStateMaterial)
    {
        ivMaterial = new SoBaseColor;
        ((SoBaseColor*)ivMaterial)->rgb.setValue(ivStateMaterial->diffuseColor[0]); // copy first value
    ...
    for (psetIndex = 0; psetIndex < numPsets; psetIndex++)
    {
        ...
        case GL_LINES:
        case GL_LINE_STRIP:
        case GL_LINE_LOOP:      if (useIndices)
    ...
    if (indexedRoot)
        ivState->ivHead->addChild(indexedRoot);

    if (nonIndexedRoot)
        ivState->ivHead->addChild(nonIndexedRoot);
...
public:
    ...
    MyShapeVisitor(InventorState *ivState)
    {
...
void ConvertToInventor::processDrawable(osg::Drawable *d)
{
    ...
    else if ((sd = dynamic_cast<osg::ShapeDrawable*>(d)) != NULL)
    {
