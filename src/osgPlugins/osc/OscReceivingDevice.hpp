...
OscReceivingDevice* getDevice() const
{
    return _device;
}
...
{
    return getDevice()->getEventQueue()->getTime();
}
private:
...
{
    return "OSC receiving device";
}

void addHandleOnCheckEvents(RequestHandler *handler)
{
    _handleOnCheckEvents.push_back(handler);
}
