...
dxfCircle::drawScene(scene *sc)
{
    ...
    if (numsteps < 3)
        numsteps = 3;           // Sanity check: minimal representation of a circle is a tri
...
dxfText::drawScene(scene *sc)
{
    ...
    if (_flags & 2)
        qr = Quat(PI, Y_AXIS) * qr;

    if (_flags & 4)
        qr = Quat(PI, X_AXIS) * qr;

    _text->setAxisAlignment(osgText::Text::USER_DEFINED_ROTATION);
    _text->setRotation(qr);

    if (_hjustify != 0 || _vjustify != 0)
        _point1 = _point2;
