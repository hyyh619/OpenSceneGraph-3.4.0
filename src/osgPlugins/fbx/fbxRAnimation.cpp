                                       ...
                                       const char *targetName, const char *channelName)
{
    ...
    if (curveX && curveX->KeyGetCount())
        interpolationType = curveX->KeyGetInterpolation(0);
    else if (curveY && curveY->KeyGetCount())
        interpolationType = curveY->KeyGetInterpolation(0);
    else if (curveZ && curveZ->KeyGetCount())
        interpolationType = curveZ->KeyGetInterpolation(0);
    ...
    const char *pTakeName)
{
        ...
        pScaleChannel)
    {
        ...
        if (pTranslationChannel)
            pAnimation->addChannel(pTranslationChannel);

        if (pRotationChannels[0])
            pAnimation->addChannel(pRotationChannels[0]);

        if (pRotationChannels[1])
            pAnimation->addChannel(pRotationChannels[1]);

        if (pRotationChannels[2])
            pAnimation->addChannel(pRotationChannels[2]);

        if (pScaleChannel)
            pAnimation->addChannel(pScaleChannel);
    ...
    else
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            if (curve && curve->KeyGetCount())
                interpolationType = curve->KeyGetInterpolation(0);
