                           ...
                           bool lightmapTextures)
{
    ...
    if (bNormal)
        pGeometry->setNormalArray(createVec3Array((precision&osgDB::Options::DOUBLE_PRECISION_NORMAL) != 0), osg::Array::BIND_PER_VERTEX);
    ...
    const char *szName)
{
    ...
    if (!layerElementValid(pFbxNormals))
        pFbxNormals = 0;

    if (!layerElementValid(pFbxColors))
        pFbxColors = 0;

    if (!layerElementValid(pFbxUVs_diffuse))
        pFbxUVs_diffuse = 0;

    if (!layerElementValid(pFbxUVs_opacity))
        pFbxUVs_opacity = 0;

    if (!layerElementValid(pFbxUVs_emissive))
        pFbxUVs_emissive = 0;

    if (!layerElementValid(pFbxUVs_ambient))
        pFbxUVs_ambient = 0;
        ...
        for (unsigned i = 0; i < pGeode->getNumDrawables(); ++i)
        {
            ...
            for (int nBlendShape = 0; nBlendShape < nDeformerBlendShapeCount; ++nBlendShape)
            {
                ...
                for (int nBlendShapeChannel = 0; nBlendShapeChannel < nBlendShapeChannelCount; ++nBlendShapeChannel)
                {
                    ...
                    if (!pBlendShapeChannel->GetTargetShapeCount())
                        continue;
                    ...
                    if (const FbxLayer *pFbxShapeLayer = pMorphShape->GetLayer(0))
                    {
                        ...
                        if (!layerElementValid(pFbxShapeNormals))
                            pFbxShapeNormals = 0;
                    }
