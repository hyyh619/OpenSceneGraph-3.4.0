...
static int callVectorResize(lua_State *_lua)
{
    ...
    if (n < 2 || lua_type(_lua, 1) != LUA_TTABLE || lua_type(_lua, 2) != LUA_TNUMBER)
        return 0;
...
static int callMapIteratorAdvance(lua_State *_lua)
{
    ...
    if (lua_gettop(_lua) < 1 || lua_type(_lua, 1) != LUA_TTABLE)
        return 0;
...
static int setMapIteratorElement(lua_State *_lua)
{
    ...
    if (lua_gettop(_lua) < 2 || lua_type(_lua, 1) != LUA_TTABLE)
        return 0;
...
static int convertStringToStateAttributeValue(const std::string&valueString, osg::StateAttribute::OverrideValue defaultValue, bool&setOnOff)
{
    ...
    if (valueString.find("ON") != std::string::npos)
    {
        value = osg::StateAttribute::ON; setOnOff = true;
    }

    if (valueString.find("OFF") != std::string::npos)
    {
        value = osg::StateAttribute::OFF; setOnOff = true;
    }

    if (valueString.find("OVERRIDE") != std::string::npos)
        value = value | osg::StateAttribute::OVERRIDE;

    if (valueString.find("PROTECTED") != std::string::npos)
        value = value | osg::StateAttribute::PROTECTED;

    if (valueString.find("INHERIT") != std::string::npos)
        value = value | osg::StateAttribute::INHERIT;
...
static std::string convertStateAttributeValueToString(unsigned int value, bool withOnOffCheck)
{
    ...
    if (withOnOffCheck)
    {
        ...
        else
        {
            ...
            valueString.append("OFF");
        }
    }

    if ((value&osg::StateAttribute::OVERRIDE) != 0)
    {
        if (!valueString.empty())
            valueString.append(", ");

        valueString.append("OVERRIDE");
    }

    if ((value&osg::StateAttribute::PROTECTED) != 0)
    {
        if (!valueString.empty())
            valueString.append(", ");

        valueString.append("PROTECTED");
    }

    if ((value&osg::StateAttribute::INHERIT) != 0)
    {
        if (!valueString.empty())
            valueString.append(", ");

        valueString.append("INHERIT");
    }
        ...
        if (sa)
        {
            if (setOnOff)
            {
                if (sa->isTextureAttribute())
                    stateset->setTextureAttributeAndModes(0, sa, value);
                else
                    stateset->setAttributeAndModes(sa, value);
            }
            else
            {
                if (sa->isTextureAttribute())
                    stateset->setTextureAttribute(0, sa, value);
                else
                    stateset->setAttribute(sa, value);
            }
...
static int callImageAllocate(lua_State *_lua)
{
    ...
    if (n >= 2 && lua_isnumber(_lua, 2))
        s = static_cast<int>(lua_tonumber(_lua, 2));

    if (n >= 3 && lua_isnumber(_lua, 3))
        t = static_cast<int>(lua_tonumber(_lua, 3));

    if (n >= 4 && lua_isnumber(_lua, 4))
        r = static_cast<int>(lua_tonumber(_lua, 4));

    if (n >= 5)
    {
        if (lua_isnumber(_lua, 5))
            pixelFormat = static_cast<int>(lua_tonumber(_lua, 5));
    ...
    if (n >= 6)
    {
        if (lua_isnumber(_lua, 6))
            dataType = static_cast<int>(lua_tonumber(_lua, 6));
    ...
    if (n >= 7)
    {
        if (lua_isnumber(_lua, 7))
            packing = static_cast<int>(lua_tonumber(_lua, 7));
    }
...
static int callImageGet(lua_State *_lua)
{
    ...
    int k = 0;
    if (n >= 2 && lua_isnumber(_lua, 2))
        i = static_cast<int>(lua_tonumber(_lua, 2));

    if (n >= 3 && lua_isnumber(_lua, 3))
        j = static_cast<int>(lua_tonumber(_lua, 3));

    if (n >= 4 && lua_isnumber(_lua, 4))
        k = static_cast<int>(lua_tonumber(_lua, 4));
    ...
    case (GL_BYTE):
        ...
        {
            colour[i] = static_cast<double>(*(reinterpret_cast<const char*>(ptr) + i));
        }
    ...
    case (GL_UNSIGNED_BYTE):
        ...
        for (unsigned int i = 0; i < numComponents; ++i)
        {
            colour[i] = static_cast<double>(*(ptr + i));
        }
    ...
    case (GL_SHORT):
        ...
        for (unsigned int i = 0; i < numComponents; ++i)
        {
            colour[i] = static_cast<double>(*(reinterpret_cast<const short*>(ptr) + i));
        }
    ...
    case (GL_UNSIGNED_SHORT):
        ...
        for (unsigned int i = 0; i < numComponents; ++i)
        {
            colour[i] = static_cast<double>(*(reinterpret_cast<const unsigned short*>(ptr) + i));
        }
    ...
    case (GL_INT):
        ...
        for (unsigned int i = 0; i < numComponents; ++i)
        {
            colour[i] = static_cast<double>(*(reinterpret_cast<const int*>(ptr) + i));
        }
    ...
    case (GL_UNSIGNED_INT):
        ...
        for (unsigned int i = 0; i < numComponents; ++i)
        {
            colour[i] = static_cast<double>(*(reinterpret_cast<const unsigned int*>(ptr) + i));
        }
    ...
    case (GL_FLOAT):
        ...
        for (unsigned int i = 0; i < numComponents; ++i)
        {
            colour[i] = static_cast<double>(*(reinterpret_cast<const float*>(ptr) + i));
        }
    ...
    case (GL_DOUBLE):
        ...
        for (unsigned int i = 0; i < numComponents; ++i)
        {
            colour[i] = static_cast<double>(*(reinterpret_cast<const double*>(ptr) + i));
        }
...
static void setImageColour(osg::Image *image, int i, int j, int k, const osg::Vec4d&colourToWrite)
{
    ...
    case (GL_BYTE):
        ...
        for (unsigned int i = 0; i < numComponents; ++i)
        {
            *(reinterpret_cast<char*>(ptr) + i) = static_cast<char>(colourToWrite[i]);
        }
    ...
    case (GL_UNSIGNED_BYTE):
        ...
        for (unsigned int i = 0; i < numComponents; ++i)
        {
            *(reinterpret_cast<unsigned char*>(ptr) + i) = static_cast<unsigned char>(colourToWrite[i]);
        }
    ...
    case (GL_SHORT):
        ...
        for (unsigned int i = 0; i < numComponents; ++i)
        {
            *(reinterpret_cast<short*>(ptr) + i) = static_cast<short>(colourToWrite[i]);
        }
    ...
    case (GL_UNSIGNED_SHORT):
        ...
        for (unsigned int i = 0; i < numComponents; ++i)
        {
            *(reinterpret_cast<unsigned short*>(ptr) + i) = static_cast<unsigned short>(colourToWrite[i]);
        }
    ...
    case (GL_INT):
        ...
        for (unsigned int i = 0; i < numComponents; ++i)
        {
            *(reinterpret_cast<int*>(ptr) + i) = static_cast<int>(colourToWrite[i]);
        }
    ...
    case (GL_UNSIGNED_INT):
        ...
        for (unsigned int i = 0; i < numComponents; ++i)
        {
            *(reinterpret_cast<unsigned int*>(ptr) + i) = static_cast<unsigned int>(colourToWrite[i]);
        }
    ...
    case (GL_FLOAT):
        ...
        for (unsigned int i = 0; i < numComponents; ++i)
        {
            *(reinterpret_cast<float*>(ptr) + i) = static_cast<float>(colourToWrite[i]);
        }
    ...
    case (GL_DOUBLE):
        ...
        for (unsigned int i = 0; i < numComponents; ++i)
        {
            *(reinterpret_cast<double*>(ptr) + i) = static_cast<double>(colourToWrite[i]);
        }
...
static int callImageSet(lua_State *_lua)
{
    ...
    int  j           = 0;
    int  k           = 0;
    if (n >= 3 && lua_isnumber(_lua, 2))
    {
        i = static_cast<int>(lua_tonumber(_lua, 2)); positionSet = true;
    }

    if (n >= 4 && lua_isnumber(_lua, 3))
    {
        j = static_cast<int>(lua_tonumber(_lua, 3)); positionSet = true;
    }

    if (n >= 5 && lua_isnumber(_lua, 4))
    {
        k = static_cast<int>(lua_tonumber(_lua, 4)); positionSet = true;
    }
    ...
    else if (lua_istable(_lua, n))
    {
        ...
        {
            double i = lua_tonumber(_lua, -1); colour[0] = i; colour[1] = i; colour[2] = i; colour[3] = i;
        }
        ...
        if (lua_isnumber(_lua, -1))
        {
            double l = lua_tonumber(_lua, -1); colour[0] = l; colour[1] = l; colour[2] = l;
        }
        ...
        if (lua_isnumber(_lua, -1))
        {
            double a = lua_tonumber(_lua, -1); colour[3] = a;
        }
        ...
        if (lua_isnumber(_lua, -1))
        {
            double r = lua_tonumber(_lua, -1); colour[0] = r;
        }
        ...
        if (lua_isnumber(_lua, -1))
        {
            double g = lua_tonumber(_lua, -1); colour[1] = g;
        }
        ...
        if (lua_isnumber(_lua, -1))
        {
            double b = lua_tonumber(_lua, -1); colour[2] = b;
        }
...
bool LuaScriptEngine::run(osg::Script *script, const std::string&entryPoint, osg::Parameters&inputParameters, osg::Parameters&outputParameters)
{
    ...
    for (int i = 0; i < numReturns; ++i)
    {
        ...
        if (obj.valid())
            outputParameters.push_back(obj);
    }
...
PushStackValueVisitor(const LuaScriptEngine *lse) : _lse(lse)
{
    _lua = const_cast<LuaScriptEngine*>(lse)->getLuaState();
}
...
GetStackValueVisitor(const LuaScriptEngine *lse, int index) : _lse(lse), _lua(0), _index(index), _numberToPop(0)
{
    _lua = const_cast<LuaScriptEngine*>(lse)->getLuaState();
}
...
int LuaScriptEngine::getDataFromStack(SerializerScratchPad *ssp, osgDB::BaseSerializer::Type type, int pos) const
{
    ...
    if (type == osgDB::BaseSerializer::RW_UNDEFINED)
        type = LuaScriptEngine::getType(pos);
    ...
    case (osgDB::BaseSerializer::RW_OBJECT):
    {
        if (lua_istable(_lua, pos))
        {
            ...
            lua_rawget(_lua, pos);
            if (lua_type(_lua, -1) == LUA_TUSERDATA)
                value = *const_cast<osg::Object**>(reinterpret_cast<const osg::Object**>(lua_touserdata(_lua, -1)));
...
bool LuaScriptEngine::getfields(int pos, const char *f1, const char *f2, int type) const
{
    ...
    if (lua_type(_lua, -2) != type || lua_type(_lua, -1) != type)
    {
        lua_pop(_lua, 2); return false;
    }
...
bool LuaScriptEngine::getfields(int pos, const char *f1, const char *f2, const char *f3, int type) const
{
    ...
    if (lua_type(_lua, -3) != type || lua_type(_lua, -2) != type ||
        lua_type(_lua, -1) != type)
    {
        lua_pop(_lua, 3); return false;
    }
...
bool LuaScriptEngine::getfields(int pos, const char *f1, const char *f2, const char *f3, const char *f4, int type) const
{
    ...
    if (lua_type(_lua, -4) != type || lua_type(_lua, -3) != type ||
        lua_type(_lua, -2) != type || lua_type(_lua, -1) != type)
    {
        lua_pop(_lua, 4); return false;
    }
...
bool LuaScriptEngine::getelements(int pos, int numElements, int type) const
{
    ...
    for (int i = 0; i < numElements; ++i)
    {
        ...
        if (lua_type(_lua, -1) != type)
        {
            lua_pop(_lua, i + 1); return false;
        }
    }
...
osg::Object* LuaScriptEngine::popParameterObject() const
{
    ...
    {
        if (lua_isboolean(_lua, -1))
            object = new osg::BoolValueObject("", lua_toboolean(_lua, -1) != 0);
    ...
    {
        if (lua_isnumber(_lua, -1))
            object = new osg::UIntValueObject("", static_cast<unsigned int>(lua_tonumber(_lua, -1)));
    ...
    {
        if (lua_isnumber(_lua, -1))
            object = new osg::FloatValueObject("", static_cast<float>(lua_tonumber(_lua, -1)));
    ...
    {
        if (lua_isnumber(_lua, -1))
            object = new osg::DoubleValueObject("", static_cast<double>(lua_tonumber(_lua, -1)));
    ...
    case (osgDB::BaseSerializer::RW_VEC2F):
    {
        osg::Vec2f value;
        if (getValue(-1, value))
            object = new osg::Vec2fValueObject("", value);
    ...
    case (osgDB::BaseSerializer::RW_VEC3F):
    {
        osg::Vec3f value;
        if (getValue(-1, value))
            object = new osg::Vec3fValueObject("", value);
    ...
    case (osgDB::BaseSerializer::RW_VEC4F):
    {
        osg::Vec4f value;
        if (getValue(-1, value))
            object = new osg::Vec4fValueObject("", value);
    ...
    case (osgDB::BaseSerializer::RW_MATRIXF):
    {
        osg::Matrixf value;
        if (getValue(-1, value))
            object = new osg::MatrixfValueObject("", value);
    ...
    case (osgDB::BaseSerializer::RW_VEC2D):
    {
        osg::Vec2d value;
        if (getValue(-1, value))
            object = new osg::Vec2dValueObject("", value);
    ...
    case (osgDB::BaseSerializer::RW_VEC3D):
    {
        osg::Vec3d value;
        if (getValue(-1, value))
            object = new osg::Vec3dValueObject("", value);
    ...
    case (osgDB::BaseSerializer::RW_VEC4D):
    {
        osg::Vec4d value;
        if (getValue(-1, value))
            object = new osg::Vec4dValueObject("", value);
    ...
    case (osgDB::BaseSerializer::RW_QUAT):
    {
        osg::Quat value;
        if (getValue(-1, value))
            object = new osg::QuatValueObject("", value);
    ...
    case (osgDB::BaseSerializer::RW_PLANE):
    {
        osg::Plane value;
        if (getValue(-1, value))
            object = new osg::PlaneValueObject("", value);
    ...
    case (osgDB::BaseSerializer::RW_MATRIXD):
    {
        osg::Matrixd value;
        if (getValue(-1, value))
            object = new osg::MatrixdValueObject("", value);
    ...
    case (osgDB::BaseSerializer::RW_BOUNDINGBOXF):
    {
        osg::BoundingBoxf value;
        if (getValue(-1, value))
            object = new osg::BoundingBoxfValueObject("", value);
    ...
    case (osgDB::BaseSerializer::RW_BOUNDINGBOXD):
    {
        osg::BoundingBoxd value;
        if (getValue(-1, value))
            object = new osg::BoundingBoxdValueObject("", value);
    ...
    case (osgDB::BaseSerializer::RW_BOUNDINGSPHEREF):
    {
        osg::BoundingSpheref value;
        if (getValue(-1, value))
            object = new osg::BoundingSpherefValueObject("", value);
    ...
    case (osgDB::BaseSerializer::RW_BOUNDINGSPHERED):
    {
        osg::BoundingSphered value;
        if (getValue(-1, value))
            object = new osg::BoundingSpheredValueObject("", value);
...
void LuaScriptEngine::createAndPushObject(const std::string&compoundName) const
{
    ...
    if (!object)
        OSG_NOTICE << "Failed to create object " << compoundName << std::endl;
