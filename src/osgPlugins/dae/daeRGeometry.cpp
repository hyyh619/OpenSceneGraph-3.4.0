    ...
    for (size_t i = 0; i < inputs.getCount(); i++)
    {
        if (strcmp(COMMON_PROFILE_INPUT_VERTEX, inputs[i]->getSemantic()) == 0)
        {
            ...
            if (domVertices *vertices = daeSafeCast<domVertices>(pDaeElement))
            {
                ...
                if (color_source)
                    color_offset = position_offset;

                if (normal_source)
                    normal_offset = position_offset;

                if (texcoord_sources[0])
                    texcoord_offsets[0] = position_offset;
            }
...
struct VertexIndices
{
    ...
    bool operator <(const VertexIndices &rhs) const
    {
        ...
        if (color_index != rhs.color_index)
            return color_index < rhs.color_index;

        if (normal_index != rhs.normal_index)
            return normal_index < rhs.normal_index;

        for (unsigned int i = 0; i < MAX_TEXTURE_COORDINATE_SETS; ++i)
        {
            if (texcoord_indices[i] != rhs.texcoord_indices[i])
                return texcoord_indices[i] < rhs.texcoord_indices[i];
        }
...
template<>
inline int VertexIndices::get<VertexIndices::POSITION>() const
{
    return position_index;
}
template<>
inline int VertexIndices::get<VertexIndices::COLOR>() const
{
    return color_index;
}
template<>
inline int VertexIndices::get<VertexIndices::NORMAL>() const
{
    return normal_index;
}
template<int Value>
...
inline osg::Array* createGeometryArray(domSourceReader &sourceReader, const VertexIndicesIndexMap &vertexIndicesIndexMap, bool useDoublePrecision, int texcoordNum = -1)
{
    if (useDoublePrecision)
        ...
        return createGeometryArray<ArrayTypeSingle, Value>(sourceReader, vertexIndicesIndexMap, texcoordNum);
}
