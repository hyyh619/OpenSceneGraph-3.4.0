...
ArrayType getArrayType(bool enableDoublePrecision) const
{
    if (srcInit)
        const_cast<domSourceReader*>(this)->convert(enableDoublePrecision);
...
int getCount(bool enableDoublePrecision) const
{
    if (srcInit)
        const_cast<domSourceReader*>(this)->convert(enableDoublePrecision);
...
{
    ASSERT_TYPE(Float); return (*m_float_array)[index];
};

osg::Vec2 const&getVec2(int index)
{
    ASSERT_TYPE(Vec2); return (*m_vec2_array)[index];
};

osg::Vec3 const&getVec3(int index)
{
    ASSERT_TYPE(Vec3); return (*m_vec3_array)[index];
};

osg::Vec4 const&getVec4(int index)
{
    ASSERT_TYPE(Vec4); return (*m_vec4_array)[index];
};

osg::Vec2d const&getVec2d(int index)
{
    ASSERT_TYPE(Vec2d); return (*m_vec2d_array)[index];
};

osg::Vec3d const&getVec3d(int index)
{
    ASSERT_TYPE(Vec3d); return (*m_vec3d_array)[index];
};

osg::Vec4d const&getVec4d(int index)
{
    ASSERT_TYPE(Vec4d); return (*m_vec4d_array)[index];
};

osg::Matrixf const&getMatrix(int index)
{
    ASSERT_TYPE(Matrix); return (*m_matrix_array)[index];
};
