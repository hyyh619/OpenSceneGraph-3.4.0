...
const std::string&getAssetUnitName() const
{
    return _assetUnitName;
}
float getAssetUnitMeter() const
{
    return _assetUnitMeter;
}
domUpAxisType getAssetUpAxis() const
{
    return _assetUp_axis;
}
...
bool operator <(const TextureParameters &rhs) const
{
    ...
    if (wrap_s != rhs.wrap_s)
        return wrap_s < rhs.wrap_s;

    if (wrap_t != rhs.wrap_t)
        return wrap_t < rhs.wrap_t;

    if (filter_min != rhs.filter_min)
        return filter_min < rhs.filter_min;

    if (filter_mag != rhs.filter_mag)
        return filter_mag < rhs.filter_mag;

    if (transparency != rhs.transparency)
        return transparency < rhs.transparency;

    if (opaque != rhs.opaque)
        return opaque < rhs.opaque;

    if (transparent != rhs.transparent)
        return transparent < rhs.transparent;
...
bool isJoint(const domNode *node) const
{
    return _jointSet.find(node) != _jointSet.end();
}
