...
void trpgwGeomHelper::EndPolygon()
{
    ...
    case trpgGeometry::Triangles:
    {
        ...
        for (int i = 0; i < num; i++)
        {
            ...
            for (loop = 0; loop < numMats; loop++)
                tex.push_back(polyTex[loop]);

            for (loop = 0; loop < numMats; loop++)
                tex.push_back(polyTex[numMats * id1 + loop]);

            for (loop = 0; loop < numMats; loop++)
                tex.push_back(polyTex[numMats * id2 + loop]);
        }
    }
    break;
    ...
    case trpgGeometry::Quads:
    {
        ...
        if (polyVert.size() == 4)
        {
            for (int i = 0; i < num; i++)
            {
                ...
                // multiple textures
                for (unsigned int loop = 0; loop < numMats; loop++)
                    tex.push_back(polyTex[numMats * i + loop]);
            }
        }
    }
    break;
    ...
    case trpgGeometry::Quads:
    {
        ...
        if (numVert % 4 == 0)
        {
            ...
            for (unsigned int i = 0; i < numVert; i++)
            {
                ...
                for (loop = 0; loop < numMat; loop++)
                    quads.AddTexCoord((trpgGeometry::DataType)dtype, tex[i * numMat + loop], loop);
            }

            quads.SetNumPrims(numVert / 4);

            for (loop = 0; loop < numMat; loop++)
                quads.AddMaterial(matTri[loop]);
...
optVert::optVert(int numMat, int vid, std::vector<trpg3dPoint> &iv, std::vector<trpg3dPoint> &in, std::vector<trpg2dPoint> &itex)
{
    ...
    for (unsigned int loop = 0; loop < (unsigned int)numMat; loop++)
        tex.push_back(itex[vid * numMat + loop]);
}
