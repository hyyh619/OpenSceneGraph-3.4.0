...
{
public:
BinaryOutputIterator(std::ostream *ostream)
{
    _out = ostream;
}
...
virtual void readLong(long &l)
{
    ...
    if (_byteSwap)
        osg::swapBytes((char*)&value, osgDB::LONG_SIZE);
...
virtual void readGLenum(osgDB::ObjectGLenum &value)
{
    ...
    if (_byteSwap)
        osg::swapBytes((char*)&e, osgDB::GLENUM_SIZE);
...
virtual void readProperty(osgDB::ObjectProperty &prop)
{
    ...
    if (prop._mapProperty)
    {
        ...
        if (_byteSwap)
            osg::swapBytes((char*)&value, osgDB::INT_SIZE);
    }
...
virtual void readMark(osgDB::ObjectMark &mark)
{
    if (_supportBinaryBrackets)
    {
        if (mark._name == "{")
        {
            ...
            if (_byteSwap)
                osg::swapBytes((char*)&size, osgDB::INT_SIZE);
