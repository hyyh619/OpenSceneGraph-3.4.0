...
void addToCurrentNode(std::ostream& (*fn)(std::ostream &))
{
    if (_nodePath.size() > 0)
    {
        ...
        _sstream << fn;
        if (_readLineType == TEXT_LINE)
            node->properties["text"] += _sstream.str();
        else
            node->properties["attribute"] += _sstream.str();
...
{
    std::string str; if (prepareStream())
        _sstream >> str;

    s = static_cast<short>(strtol(str.c_str(), NULL, 0));
}
...
virtual void readUShort(unsigned short &s)
{
    ...
    s = static_cast<unsigned short>(strtoul(str.c_str(), NULL, 0));
}
...
virtual void readInt(int &i)
{
    ...
    i = static_cast<int>(strtol(str.c_str(), NULL, 0));
}
...
virtual void readUInt(unsigned int &i)
{
    ...
    i = static_cast<unsigned int>(strtoul(str.c_str(), NULL, 0));
}
...
{
    return _sstream.rdbuf()->in_avail() > 0;
}
...
bool prepareStream()
{
    ...
    if (current->type != osgDB::XmlNode::COMMENT)
    {
        ...
        {
            if (applyPropertyToStream(current, "attribute"))
                return true;
            else if (applyPropertyToStream(current, "text"))
                return true;
        }
