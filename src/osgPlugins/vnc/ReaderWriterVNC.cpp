...
double getTimeOfLastRender() const
{
    return _timeOfLastRender;
}
...
bool LibVncImage::connect(const std::string &hostname)
{
    ...
    if (!_password.empty())
        _client->GetPassword = getPassword;
...
rfbBool LibVncImage::resizeImage(rfbClient *client)
{
    ...
    {
        if (image->_optionString.find("swap") != std::string::npos || image->_optionString.find("swop") != std::string::npos)
            swap = true;
    }
    ...
    if (!image->_optionString.empty())
    {
        if (image->_optionString.find("RGB") != std::string::npos)
            gl_pixelFormat = GL_RGBA;

        if (image->_optionString.find("RGBA") != std::string::npos)
            gl_pixelFormat = GL_RGBA;

        if (image->_optionString.find("BGR") != std::string::npos)
            gl_pixelFormat = GL_BGRA;

        if (image->_optionString.find("BGRA") != std::string::npos)
            gl_pixelFormat = GL_BGRA;
    }
