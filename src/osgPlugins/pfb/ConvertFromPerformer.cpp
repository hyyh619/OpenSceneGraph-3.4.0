...
osg::Node* ConvertFromPerformer::visitNode(osg::Group *osgParent, pfNode *node)
{
    ...
    if (node->getType()->isDerivedFrom(pfBillboard::getClassType()))
        return visitBillboard(osgParent, (pfBillboard*)node);
    else if (node->getType()->isDerivedFrom(pfGeode::getClassType()))
        return visitGeode(osgParent, (pfGeode*)node);
    else if (node->getType()->isDerivedFrom(pfScene::getClassType()))
        return visitScene(osgParent, (pfScene*)node);
    else if (node->getType()->isDerivedFrom(pfDCS::getClassType()))
        return visitDCS(osgParent, (pfDCS*)node);
    else if (node->getType()->isDerivedFrom(pfSCS::getClassType()))
        return visitSCS(osgParent, (pfSCS*)node);
    else if (node->getType()->isDerivedFrom(pfLOD::getClassType()))
        return visitLOD(osgParent, (pfLOD*)node);
    else if (node->getType()->isDerivedFrom(pfSequence::getClassType()))
        return visitSequence(osgParent, (pfSequence*)node);
    else if (node->getType()->isDerivedFrom(pfSwitch::getClassType()))
        return visitSwitch(osgParent, (pfSwitch*)node);
    else if (node->getType()->isDerivedFrom(pfGroup::getClassType()))
        return visitGroup(osgParent, (pfGroup*)node);
                                               ...
                                               pfSequence *sequence)
{
    ...
    {
        if (osgParent)
            osgParent->addChild(osgSequence);
...
osg::Node* ConvertFromPerformer::visitDCS(osg::Group *osgParent, pfDCS *dcs)
{
    ...
    {
        if (osgParent)
            osgParent->addChild(osgTransform);
...
osg::Node* ConvertFromPerformer::visitBillboard(osg::Group *osgParent, pfBillboard *billboard)
{
    ...
    {
        if (osgParent)
            osgParent->addChild(osgBillboard);
...
osg::Drawable* ConvertFromPerformer::visitGeoSet(osg::Geode *osgGeode, pfGeoSet *geoset)
{
    ...
    if (prim == PFGS_FLAT_LINESTRIPS)
        flat_shaded_skip_per_primitive = 1;
    else if (prim == PFGS_FLAT_TRISTRIPS)
        flat_shaded_skip_per_primitive = 2;
    else if (prim == PFGS_FLAT_TRIFANS)
        flat_shaded_skip_per_primitive = 2;
...
osg::StateSet* ConvertFromPerformer::visitGeoState(osg::Drawable *osgDrawable, pfGeoState *geostate)
{
    ...
    {
        if (osgDrawable)
            osgDrawable->setStateSet(osgStateSet);
...
//                break;
    ...
    if (inherit & PFSTATE_TRANSPARENCY)
        osgStateSet->setMode(GL_BLEND, osg::StateAttribute::INHERIT);
    else
    {
...
//            case(PFTR_NO_OCCLUDE): //this is a bit flag.
    ...
    if (inherit & PFSTATE_ENTEXTURE)
        osgStateSet->setTextureMode(0, GL_TEXTURE_2D, osg::StateAttribute::INHERIT);
    else
    {
    ...
    if (inherit & PFSTATE_CULLFACE)
        osgStateSet->setMode(GL_CULL_FACE, osg::StateAttribute::INHERIT);
    else
    {
    ...
    if (inherit & PFSTATE_ENLIGHTING)
        osgStateSet->setMode(GL_LIGHTING, osg::StateAttribute::INHERIT);
    else
    {
    ...
    if (inherit & PFSTATE_ENFOG)
        osgStateSet->setMode(GL_FOG, osg::StateAttribute::INHERIT);
    else
    {
...
{
    if (front_mat == NULL && back_mat == NULL)
        return NULL;
    ...
    if (front_mat == back_mat)
        material = front_mat;
    else if (back_mat == NULL)
        material = front_mat;
    else if (front_mat == NULL)
        material = back_mat;
...
osg::Texture2D* ConvertFromPerformer::visitTexture(osg::StateSet *osgStateSet, pfTexture *tex)
{
    ...
    {
        if (osgStateSet)
            osgStateSet->setTextureAttribute(0, osgTexture);
    ...
    if (!_saveAbsoluteImagePath)
        texName = osgDB::getSimpleFileName(texName);
