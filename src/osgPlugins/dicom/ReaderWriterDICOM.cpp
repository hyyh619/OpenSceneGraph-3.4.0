...
virtual ReadResult readImage(const std::string &file, const osgDB::ReaderWriter::Options *options) const
{
         ...
         ++itr)
    {
        ...
        if (result.success())
            images.push_back(result.getImage());
        else
            return result;
    }
...
struct SeriesIdentifier
{
    ...
    bool operator ==(const SeriesIdentifier &rhs) const
    {
        ...
        if (SeriesDescription != rhs.SeriesDescription)
            return false;

        for (unsigned int i = 0; i < 6; ++i)
        {
            if (Orientation[i] != rhs.Orientation[i])
                return false;
        }
    ...
    bool operator <(const SeriesIdentifier &rhs) const
    {
        ...
        if (rhs.SeriesInstanceUID < SeriesInstanceUID)
            return false;

        if (SeriesDescription < rhs.SeriesDescription)
            return true;

        if (rhs.SeriesDescription < SeriesDescription)
            return false;
