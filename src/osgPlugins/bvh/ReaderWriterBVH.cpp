...
osg::Group* buildBVH(std::istream&stream, const osgDB::ReaderWriter::Options *options)
{
    if (options)
    {
        if (options->getOptionString().find("contours") != std::string::npos)
            _drawingFlag = 1;
        else if (options->getOptionString().find("solids") != std::string::npos)
            _drawingFlag = 2;
    }
...
void alterChannel(std::string name, int&value)
{
    if (name == "Xposition")
        ...
        value |= 0x04;
    else if (name == "Zrotation")
        value |= 0x08;
    else if (name == "Xrotation")
        value |= 0x10;
    else if (name == "Yrotation")
        value |= 0x20;
}
                 ...
                 osgAnimation::QuatKeyframeContainer *rotKey)
{
    if (ch & 0x07 && posKey)      // Position keyframe
    {
        ...
        if (ch & 0x01)
            fr.readSequence(keyValue[0]);

        if (ch & 0x02)
            fr.readSequence(keyValue[1]);

        if (ch & 0x04)
            fr.readSequence(keyValue[2]);
