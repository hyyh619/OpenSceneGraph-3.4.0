...
LP_TMP iff::Chunk*Parser<Iter>::parse_chunk_data(const std::string &tag, const std::string &context, Iter it, Iter end)
{
    ...
    if (context == "FORM::ENVL")
    {
        ...
        if (tag == "SPAN")
        {
            ...
            while (it < end)
                chk->parameters.push_back(read_F4(it));
    ...
    if (context == "FORM::SURF")
    {
        ...
        if (tag == "BLOK")
        {
            ...
            for (Iter tempit = it; tempit < (it + 4); ++tempit)
                hid += *tempit;
    ...
    if (context == "FORM::SURF::BLOK::IMAP" || context == "FORM::SURF::BLOK::PROC")
    {
        if (tag == "TMAP")
        {
            ...
            while (it < end)
                chk->attributes.push_back(parse_subchunk(it, "FORM::SURF::BLOK::IMAP/PROC::TMAP"));
    ...
    if (context == "FORM::SURF::BLOK::PROC")
    {
        ...
        if (tag == "VALU")
        {
            ...
            while (it < end)
                chk->value.push_back(read_FP4(it));
    ...
    if (context == "FORM::SURF::BLOK::GRAD")
    {
        ...
        if (tag == "IKEY")
        {
            ...
            while (it < end)
                chk->interpolation.push_back(read_U2(it));
...
LP_TMP iff::Chunk*Parser<Iter>::parse_subchunk(Iter &it, const std::string &context)
{
    ...
    if (!chk)
        this->os() << "DEBUG INFO: lwo2parser: \tprevious subchunk not handled\n";
