    ...
    for (Object::Layer_map::iterator li = obj.layers().begin(); li != obj.layers().end(); ++li)
    {
        ...
        for (Layer::Unit_list::iterator j = layer.units().begin(); j != layer.units().end(); ++j)
        {
            ...
            for (GeometryBin_map::iterator i = bins.begin(); i != bins.end(); ++i)
            {
                ...
                if (!bin.deui_points->empty())
                    geo->addPrimitiveSet(bin.deui_points.get());

                if (!bin.deui_lines->empty())
                    geo->addPrimitiveSet(bin.deui_lines.get());

                if (!bin.deui_triangles->empty())
                    geo->addPrimitiveSet(bin.deui_triangles.get());
            }
        }
