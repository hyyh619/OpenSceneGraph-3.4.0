                     ...
                     int headerlen)
{
    ...
    if (memcmp((const void*)header, (const void*)tifcmp, 4) == 0)
        return 1;

    if (memcmp((const void*)header, (const void*)tifcmp2, 4) == 0)
        return 1;
                 ...
                 uint16&bitspersample)
{
    ...
    case pack(PHOTOMETRIC_RGB, PLANARCONFIG_SEPARATE):
        ...
        for (row = 0; !tifferror && row < h; row++)
        {
            ...
            {
                if (format == 3)
                    interleave_row(currPtr, inbuf, inbuf + rowsize, inbuf + 2 * rowsize, w, format, bitspersample);
                else if (format == 4)
                    interleave_row(currPtr, inbuf, inbuf + rowsize, inbuf + 2 * rowsize, inbuf + 3 * rowsize, w, format, bitspersample);
...
virtual bool acceptsExtension(const std::string&extension) const
{
    ...
    if (osgDB::equalCaseInsensitive(extension, "tif"))
        return true;
