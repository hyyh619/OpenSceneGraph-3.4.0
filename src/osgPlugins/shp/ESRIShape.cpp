...
{
    return _read(fd, buf, static_cast<unsigned int>(nbytes));
}
}
...
{
    if (readVal<Double>(fd, Xmin, LittleEndian) == false)
        return false;

    if (readVal<Double>(fd, Ymin, LittleEndian) == false)
        return false;

    if (readVal<Double>(fd, Xmax, LittleEndian) == false)
        return false;

    if (readVal<Double>(fd, Ymax, LittleEndian) == false)
        return false;

    if (readVal<Double>(fd, Zmin, LittleEndian) == false)
        return false;

    if (readVal<Double>(fd, Zmax, LittleEndian) == false)
        return false;

    if (readVal<Double>(fd, Mmin, LittleEndian) == false)
        return false;

    if (readVal<Double>(fd, Mmax, LittleEndian) == false)
        return false;
...
{
    if (readVal<Integer>(fd, fileCode, BigEndian) == false)
        return false;

    if (esri::read(fd, _unused_0, sizeof(_unused_0)) <= 0)
        return false;

    if (readVal<Integer>(fd, fileLength, BigEndian) == false)
        return false;

    if (readVal<Integer>(fd, version, LittleEndian) == false)
        return false;
...
{
    if (readVal<Integer>(fd, recordNumber, BigEndian) == false)
        return false;

    if (readVal<Integer>(fd, contentLength, BigEndian) == false)
        return false;
...
{
    if (readVal<Double>(fd, min, LittleEndian) == false)
        return false;

    if (readVal<Double>(fd, max, LittleEndian) == false)
        return false;
