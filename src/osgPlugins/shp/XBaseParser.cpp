...
bool XBaseHeader::read(int fd)
{
    ...
    if ((nbytes = ::read(fd, &_lastUpdate, sizeof(_lastUpdate))) <= 0)
        return false;

    if ((nbytes = ::read(fd, &_numRecord, sizeof(_numRecord))) <= 0)
        return false;
    ...
    if ((nbytes = ::read(fd, &_reserved, sizeof(_reserved))) <= 0)
        return false;

    if ((nbytes = ::read(fd, &_incompleteTransaction, sizeof(_incompleteTransaction))) <= 0)
        return false;

    if ((nbytes = ::read(fd, &_encryptionFlag, sizeof(_encryptionFlag))) <= 0)
        return false;

    if ((nbytes = ::read(fd, &_freeRecordThread, sizeof(_freeRecordThread))) <= 0)
        return false;

    if ((nbytes = ::read(fd, &_reservedMultiUser, sizeof(_reservedMultiUser))) <= 0)
        return false;

    if ((nbytes = ::read(fd, &_mdxflag, sizeof(_mdxflag))) <= 0)
        return false;

    if ((nbytes = ::read(fd, &_languageDriver, sizeof(_languageDriver))) <= 0)
        return false;

    if ((nbytes = ::read(fd, &_reserved2, sizeof(_reserved2))) <= 0)
        return false;
...
bool XBaseFieldDescriptor::read(int fd)
{
    ...
    if ((nbytes = ::read(fd, &_name, sizeof(_name))) <= 0)
        return false;

    if ((nbytes = ::read(fd, &_fieldType, sizeof(_fieldType))) <= 0)
        return false;

    if ((nbytes = ::read(fd, &_fieldDataAddress, sizeof(_fieldDataAddress))) <= 0)
        return false;
    ...
    if ((nbytes = ::read(fd, &_workAreaID, sizeof(_workAreaID))) <= 0)
        return false;

    if ((nbytes = ::read(fd, &_reservedMultiUser2, sizeof(_reservedMultiUser2))) <= 0)
        return false;
...
//    _xBaseHeader.print();
    ...
    while (fieldDescriptorDone == false)
    {
        ...
        if (_xBaseFieldDescriptorTmp.read(fd) == false)
            return false;
...
//        _xBaseFieldDescriptorTmp.print();
        ...
        if ((nbytes = ::read(fd, &nullTerminator, sizeof(nullTerminator))) <= 0)
            return false;
