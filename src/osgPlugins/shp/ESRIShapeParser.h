...
{
    if (useDouble)
        _vec3darray = new osg::Vec3dArray;
    else
        _vec3farray = new osg::Vec3Array;
}

osg::Array* get()
{
    return _vec3farray.valid() ?
...
void add(double x, double y, double z)
{
    if (_vec3farray.valid())
        _vec3farray->push_back(osg::Vec3(x, y, z));
    else
...
void add(osg::Array *array, unsigned int index)
{
    ...
    if (vec3Array && index < vec3Array->size())
        add((*vec3Array)[index]);
