...
bool ReaderWriterP3DXML::getProperties(osgDB::XmlNode *cur, osgPresentation::SlideShowConstructor::PositionData &value) const
{
    ...
    if (getProperty(cur, "coordinate_frame", str))
    {
        ...
        if (match(str, "model"))
            value.frame = osgPresentation::SlideShowConstructor::MODEL;
        else if (match(str, "slide"))
            value.frame = osgPresentation::SlideShowConstructor::SLIDE;
        else
            OSG_NOTIFY(_notifyLevel) << "Parser error - coordinate_frame=\"" << str << "\" unrecongonized value" << std::endl;
    ...
    if (value.frame == osgPresentation::SlideShowConstructor::SLIDE)
    {
        if (getProperty(cur, "position", str))
        {
            ...
            bool fail = false;
            if (match(str, "center"))
                value.position.set(0.5f, .5f, 0.0f);
            else if (match(str, "eye"))
                value.position.set(0.0f, 0.0f, 1.0f);
            else if (read(str, vec3))
                value.position = vec3;
            else if (read(str, vec2))
                value.position.set(vec3.x(), vec3.y(), 0.0f);
            else
                fail = true;

            if (fail)
            {
                OSG_NOTIFY(_notifyLevel) << "Parser error - position=\"" << str << "\" unrecongonized value" << std::endl;
            }
            else
            {
                OSG_NOTIFY(_notifyLevel) << "Read position=" << value.position << std::endl;
            }
        }
    }
    else // value.frame==osgPresentation::SlideShowConstructor::MODEL
    {
        if (getProperty(cur, "position", str))
        {
            ...
            bool fail = false;
            if (match(str, "center"))
                value.position.set(0.0f, 1.0f, 0.0f);
            else if (match(str, "eye"))
                value.position.set(0.0f, 0.0f, 0.0f);
            else if (!read(str, value.position))
                fail = true;
    ...
    if (getProperty(cur, "path_loop_mode", str))
    {
        ...
        if (match(str, "LOOP"))
            value.path_loop_mode = osg::AnimationPath::LOOP;
        else if (match(str, "SWING"))
            value.path_loop_mode = osg::AnimationPath::SWING;
        else if (match(str, "NO_LOOPING"))
            value.path_loop_mode = osg::AnimationPath::NO_LOOPING;
    ...
    if (getProperty(cur, "animation_material_loop_mode", str))
    {
        ...
        if (match(str, "LOOP"))
            value.animation_material_loop_mode = osgPresentation::AnimationMaterial::LOOP;
        else if (match(str, "SWING"))
            value.animation_material_loop_mode = osgPresentation::AnimationMaterial::SWING;
        else if (match(str, "NO_LOOPING"))
            value.animation_material_loop_mode = osgPresentation::AnimationMaterial::NO_LOOPING;
...
bool ReaderWriterP3DXML::getProperties(osgDB::XmlNode *cur, osgPresentation::SlideShowConstructor::ImageData &value) const
{
    ...
    if (getProperty(cur, "looping", str))
    {
        propertiesRead = true;
        if (match(str, "on"))
            value.loopingMode = osg::ImageStream::LOOPING;
        else
            value.loopingMode = osg::ImageStream::NO_LOOPING;
    ...
    {
        propertiesRead = true;
        if (match(str, "PRE_LOAD_ALL_IMAGES"))
            value.imageSequencePagingMode = osg::ImageSequence::PRE_LOAD_ALL_IMAGES;
        else if (match(str, "PAGE_AND_RETAIN_IMAGES"))
            value.imageSequencePagingMode = osg::ImageSequence::PAGE_AND_RETAIN_IMAGES;
        else if (match(str, "PAGE_AND_DISCARD_USED_IMAGES"))
            value.imageSequencePagingMode = osg::ImageSequence::PAGE_AND_DISCARD_USED_IMAGES;
        else if (match(str, "LOAD_AND_RETAIN_IN_UPDATE_TRAVERSAL"))
            value.imageSequencePagingMode = osg::ImageSequence::LOAD_AND_RETAIN_IN_UPDATE_TRAVERSAL;
        else if (match(str, "LOAD_AND_DISCARD_IN_UPDATE_TRAVERSAL"))
            value.imageSequencePagingMode = osg::ImageSequence::LOAD_AND_DISCARD_IN_UPDATE_TRAVERSAL;
    ...
    if (getProperty(cur, "interaction_mode", str))
    {
        ...
        if (match(str, "PLAY_AUTOMATICALLY_LIKE_MOVIE"))
            value.imageSequenceInteractionMode = osgPresentation::SlideShowConstructor::ImageData::PLAY_AUTOMATICALLY_LIKE_MOVIE;
        else if (match(str, "USE_MOUSE_X_POSITION"))
            value.imageSequenceInteractionMode = osgPresentation::SlideShowConstructor::ImageData::USE_MOUSE_X_POSITION;
        else if (match(str, "USE_MOUSE_Y_POSITION"))
            value.imageSequenceInteractionMode = osgPresentation::SlideShowConstructor::ImageData::USE_MOUSE_Y_POSITION;
    ...
    if (getProperty(cur, "blending", str))
    {
        ...
        if (match(str, "on") || match(str, "enable"))
            value.blendingHint = osgPresentation::SlideShowConstructor::ImageData::ON;
        else
            value.blendingHint = osgPresentation::SlideShowConstructor::ImageData::OFF;
...
void ReaderWriterP3DXML::parseVolume(osgPresentation::SlideShowConstructor &constructor, osgDB::XmlNode *cur) const
{
    ...
    {
        if (match(technique, "standard"))
            volumeData.shadingModel = osgVolume::VolumeSettings::Standard;
        else if (match(technique, "mip"))
            volumeData.shadingModel = osgVolume::VolumeSettings::MaximumIntensityProjection;
        else if (match(technique, "isosurface") || match(technique, "iso"))
            volumeData.shadingModel = osgVolume::VolumeSettings::Isosurface;
        else if (match(technique, "light"))
            volumeData.shadingModel = osgVolume::VolumeSettings::Light;
    }

    std::string renderer;
    if (getProperty(cur, "renderer", renderer))
    {
        if (match(renderer, "FixedFunction"))
            volumeData.technique = osgVolume::VolumeSettings::FixedFunction;
        else if (match(renderer, "RayTraced"))
            volumeData.technique = osgVolume::VolumeSettings::RayTraced;
        else if (match(renderer, "MultiPass"))
            volumeData.technique = osgVolume::VolumeSettings::MultiPass;
    }
    ...
    {
        if (match(operation, "NO_COLOR_SPACE_OPERATION"))
            volumeData.colorSpaceOperation = osg::NO_COLOR_SPACE_OPERATION;
        else if (match(operation, "MODULATE_ALPHA_BY_LUMINANCE"))
            volumeData.colorSpaceOperation = osg::MODULATE_ALPHA_BY_LUMINANCE;
        else if (match(operation, "MODULATE_ALPHA_BY_COLOR"))
            volumeData.colorSpaceOperation = osg::MODULATE_ALPHA_BY_COLOR;
        else if (match(operation, "REPLACE_ALPHA_WITH_LUMINANCE"))
            volumeData.colorSpaceOperation = osg::REPLACE_ALPHA_WITH_LUMINANCE;
        else if (match(operation, "REPLACE_RGB_WITH_LUMINANCE"))
            volumeData.colorSpaceOperation = osg::REPLACE_RGB_WITH_LUMINANCE;
    }
...
osgDB::ReaderWriter::ReadResult read(const osgDB::FilePathList &filePathList, ObjectType type, const std::string &filename, const osgDB::Options *options, bool checkLocalFiles)
{
         ...
         ++itr)
    {
        ...
        {
            if (checkLocalFiles)
                result = readFileCache(type, newpath, options);
            else
                result = readServer(type, newpath, options);
        }
...
osgDB::ReaderWriter::ReadResult ReaderWriterP3DXML::readNode(osgDB::XmlNode::Input &input, osgDB::ReaderWriter::Options *options) const
{
    ...
    for (osgDB::XmlNode::Children::iterator itr = doc->children.begin();
         ...
         ++itr)
    {
        if (match((*itr)->name, "presentation"))
            root = itr->get();
    }
    ...
    if (readOnlyHoldingPage && !hasHoldingSlide)
        return 0;
...
osg::Node* ReaderWriterP3DXML::parseXmlGraph(osgDB::XmlNode *root, bool readOnlyHoldingPage, osgDB::Options *options) const
{
    ...
    if (options && !(options->getDatabasePathList().empty()))
    {
        ...
        if (readFileCallback)
            readFileCallback->_paths.push_front(pathToPresentation);
    }
         ...
         ++itr)
    {
        ...
        if (match(cur->name, "path"))
        {
            ...
            if (readFileCallback)
                readFileCallback->_paths.push_back(newpath);
            else
                options->getDatabasePathList().push_back(newpath);
        }
    }
         ...
         ++itr)
    {
        ...
        else if (!readOnlyHoldingPage && match(cur->name, "properties"))
        {
            if (!constructor.getPresentationSwitch())
                constructor.createPresentation();
