...
inline int isType(mttype t1) const
{
    return     (type == t1);
}
inline int isTextured()
{
    return     (type == TiledTexture || type == FullFace);
}
...
inline float getRepWid() const
{
    return TextureWidth;
}
inline float getRepHt() const
{
    return TextureHeight;
}
inline int isFullFace() const
{
    return type == FullFace;
}
inline int getid() const
{
    return id;
}
inline void setid(const int i)
{
    id = i;
}
...
inline void setspecular(float o)
{
    specular = o;
}
inline void setspecexp(float o)
{
    specexp = o;
}
...
{
    if (strstr(buff, "kQ3AttenuationTypeNone"))
        atyp = NONE;
    else if (strstr(buff, "kQ3AttenuationTypeInverseDistance"))
        atyp = INVERSE_DIST;

    //    else if (strstr(buff,"kQ3AttenuationTypeNone")) ;
}
void setBright(const float br)
{
    bright = br;
}
void setHalfAngleIn(const float ha)
{
    halfIn = ha;
}
void setHalfAngleOut(const float ha)
{
    halfOut = ha;
}
void setFallOff(const float fo)
{
    falloff = fo;
}
...
_face()
{
    nVertStart = 0; opening = NULL; idx = NULL; nv = 0; nop = 0; nset = 0; nrm[0] = nrm[1] = nrm[2] = 0;
}
~_face()
{
    delete[] idx;
}
void setnv(const int n)
{
    nv = n; idx = new int[n];
}
...
void move(class _face *oldop)
{
    *this = *oldop; oldop->idx = NULL;
}
inline int getnv()
{
    return nv;
}
inline int getvert(const int j)
{
    return idx[j];
}
inline int complete()
{
    return (idx && nv > 0 && nset == nv);
}                                                              // face has all defined
inline int holecomplete()
{
    if (!opening)
        return 1;                                       // no hole, so it is complete

    return opening[nop - 1].complete();
}                                          // latest opening in face has all vertices defined
int getallverts(void) const
{
    int ntot = nv;

    for (int i = 0; i < nop; i++)
        ntot += opening[i].getnv();
...
inline const int getidx(int i) const
{
    return idx[i];
}
private:
...
_dwobj()
{
    nverts = nfaces = 0; openings = NULL; faces = NULL; tmat = NULL; edges = NULL;
...
{   // replace fgets to detect EOL = char 13 as well as Creturn=10 GWM 111100
    ...
    if (nread > 0)
        clin[nread - 1] = '\0';      // null terminate and remove training blank
