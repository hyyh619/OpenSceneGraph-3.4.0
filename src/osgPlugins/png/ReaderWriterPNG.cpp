...
WriteResult::WriteStatus writePngStream(std::ostream&fout, const osg::Image&img, int compression_level) const
{
    ...
    if (bitDepth > 8 && getCpuByteOrder() == LittleEndian)
        png_set_swap(png);
...
ReadResult readPNGStream(std::istream&fin) const
{
            ...
            #endif
    {
        ...
        {
            OSG_INFO << "color == PNG_COLOR_TYPE_RGB " << std::endl;
        }

        if (color == PNG_COLOR_TYPE_GRAY)
        {
            OSG_INFO << "color == PNG_COLOR_TYPE_GRAY " << std::endl;
        }

        if (color == PNG_COLOR_TYPE_GRAY_ALPHA)
        {
            OSG_INFO << "color ==  PNG_COLOR_TYPE_GRAY_ALPHA" << std::endl;
        }
