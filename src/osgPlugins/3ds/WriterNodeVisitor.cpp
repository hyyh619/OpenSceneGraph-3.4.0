...
bool is83(const std::string&s)
{
    ...
    if (s.find_first_of("/\\") != std::string::npos)
        return false;                                                         // It should not be a path, but a filename
...
inline bool is3DSName(const std::string&s, bool extendedFilePaths, bool isNodeName)
{
    ...
    if (len > maxNameLen(extendedFilePaths, isNodeName) || len == 0)
        return false;
...
PrimitiveIndexWriter&operator =(const PrimitiveIndexWriter&)
{
    return *this;
}
...
std::string convertExt(const std::string&path, bool extendedFilePaths)
{
    ...
    if (ext == ".tiff")
        ...
        ext = ".jpg";
    else if (ext == ".jpeg2000" || ext == ".jpg2000")
        ext = ".jpc";
    ...
    for (unsigned int iMat = 0; iMat < nbMat; ++iMat)
    {
        for (MaterialMap::iterator itr = _materialMap.begin(); itr != _materialMap.end(); ++itr)
        {
            ...
            if (mat.image)
            {
                ...
                if (mat.texture_no_tile)
                    tex.flags |= LIB3DS_TEXTURE_NO_TILE;
                else
                    tex.flags &= ~LIB3DS_TEXTURE_NO_TILE;
            }
...
std::string utf8TruncateBytes(const std::string&s, std::string::size_type byteLimit)
{
    ...
    for (; it != itEnd; ++it)
    {
        ...
        if ((c & 0x80) == 0)
            itStop = it + 1;                     // 7 bits ANSI. itStop must then point after that character.
        else if ((c & 0x40) != 0)
            itStop = it;                        // UTF8 sequence start: this is also past-the-end for the previous character (ANSI or UTF8)
    }
...
std::string WriterNodeVisitor::getUniqueName(const std::string&_defaultValue, bool isNodeName, const std::string&_defaultPrefix, int currentPrefixLen)
{
    ...
    if (currentPrefixLen < 0)
        currentPrefixLen = osg::maximum(_defaultPrefix.length(), _defaultValue.length());
    ...
    if (_extendedFilePaths)
    {
        ...
        if (is3DSName(_defaultValue, _extendedFilePaths, isNodeName))
        {
            ...
            if (insertion.second)
                return _defaultValue;                          // Return if element is newly inserted in the map (else there is a naming collision)
        }
        ...
        if (_defaultValue.length() > MAX_PREFIX_LENGTH)        // Not parentPath but _defaultValue!
        {
            ...
            if (isNodeName)
                std::reverse(parentPath.begin(), parentPath.end());
            ...
            if (separator != std::string::npos)
                parentPath = parentPath.substr(0, separator);
    ...
    else
    {
        ...
        if (is3DSName(strippedName, _extendedFilePaths, isNodeName))
        {
            ...
            if (insertion.second)
                return strippedName;                          // Return if element is newly inserted in the map (else there is a naming collision)
        }
    ...
    if (currentPrefixLen > 1)
        return getUniqueName(_defaultValue, isNodeName, defaultPrefix, currentPrefixLen - 1);

    // Try with default prefix if not arleady done
    if (defaultPrefix != std::string("_"))
        return getUniqueName(_defaultValue, isNodeName, "_", 1);
                             ...
                             Lib3dsMesh        *mesh)
{
    ...
    if (texcoords)
    {
        for (MapIndices::iterator it = index_vert.begin(); it != index_vert.end(); ++it)
        {
            ...
            if (!texarray || texarray->getNumElements() == 0)
                continue;
...
WriterNodeVisitor::calcVertices(osg::Geode&geo)
{
    ...
    for (unsigned int i = 0; i < geo.getNumDrawables(); ++i)
    {
        ...
        if (g != NULL && g->getVertexArray())
            numVertice += g->getVertexArray()->getNumElements();
    }
