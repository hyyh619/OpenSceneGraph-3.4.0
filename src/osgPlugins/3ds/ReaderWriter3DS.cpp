...
static void fileio_log_func(void *self, Lib3dsLogLevel level, int indent, const char *msg)
{
    ...
    if (level == LIB3DS_LOG_ERROR)
        l = osg::WARN;
    else if (level == LIB3DS_LOG_WARN)
        l = osg::NOTICE;
    else if (level == LIB3DS_LOG_INFO)
        l = osg::INFO;
    else if (level == LIB3DS_LOG_DEBUG)
        l = osg::DEBUG_INFO;
...
osgDB::ReaderWriter::ReadResult ReaderWriter3DS::readNode(std::istream &fin,  const osgDB::ReaderWriter::Options *options) const
{
    ...
    if (options)
    {
        ...
        if (optFileName.empty())
            optFileName = options->getPluginStringData("filename");
    }
...
osg::Drawable* ReaderWriter3DS::ReaderObject::createDrawable(Lib3dsMesh *m, FaceList &faceList, const osg::Matrix *matrix, StateSetInfo &ssi, bool smoothVertexNormals)
{
    ...
    if (m->texcos)
    {
        ...
        if (ssi.lib3dsmat && *(ssi.lib3dsmat->texture1_map.name))     // valid texture = name not empty
        {
            ...
            if (tex3ds.rotation != 0)
                OSG_NOTICE << "3DS texture rotation not supported yet" << std::endl;
         ...
         ++itr, ++faceIndex)
    {
        ...
        if (matrix)
            normal = osg::Matrix::transform3x3(normal, *(params.matrix));
    ...
    if (osg_vertices.valid() && osg_vertices->size() < osg_vertices->capacity())
        osg_vertices->trim();

    if (osg_normals.valid() && osg_normals->size() < osg_normals->capacity())
        osg_normals->trim();

    if (osg_texCoords.valid() && osg_texCoords->size() < osg_texCoords->capacity())
        osg_texCoords->trim();
...
osgDB::ReaderWriter::WriteResult ReaderWriter3DS::doWriteNode(const osg::Node &node, std::ostream &fout, const Options *options, const std::string &fileNamelib3ds) const
{
    ...
    bool ok = true;
    if (!createFileObject(node, file3ds, fileNamelib3ds, local_opt.get()))
        ok = false;

    if (ok && !lib3ds_file_write(file3ds, &io))
        ok = false;
