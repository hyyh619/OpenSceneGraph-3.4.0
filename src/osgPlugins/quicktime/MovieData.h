...
inline double getMovieDuration()
{
    return GetMovieDuration(_movie) / _timescale;
}

/** @return the current position for this movie in seconds */
inline double getMovieTime()
{
    return GetMovieTime(_movie, NULL) / _timescale;
}
...
inline double getMovieRate()
{
    return Fix2X(GetMovieRate(_movie));
}
/** @return returns the cached movierate, may differ to the real movierate */
inline double getCachedMovieRate()
{
    return _movieRate;
}
...
{
    SetMovieVolume(_movie, (short)(ceil(volume * 255.0f)));
}
float getVolume() const
{
    return GetMovieVolume(_movie) / 255.0f;
}
