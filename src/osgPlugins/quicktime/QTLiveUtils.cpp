...
void print_video_component_capability(VideoDigitizerComponent aComponent)
{
    ...
    else
    {
        ...
        if (vdigType == vdTypeBasic)
        {
            OSG_NOTICE << "Digitizer Type : Basic (no clipping)" << std::endl;
        }

        if (vdigType == vdTypeAlpha)
        {
            OSG_NOTICE << "Digitizer Type : Alpha clipping" << std::endl;
        }

        if (vdigType == vdTypeMask)
        {
            OSG_NOTICE << "Digitizer Type : Mask Plane clipping" << std::endl;
        }

        if (vdigType == vdTypeKey)
        {
            OSG_NOTICE << "Digitizer Type : Key Color(s) clipping" << std::endl;
        }
        ...
        if (vid_err)
        {
            OSG_NOTICE << "VDGetPreferredImageDimensions(aComponent, &pref_width, &pref_height) - ERROR" << std::endl;
        }
        else
        {
            OSG_NOTICE << "Preferrred width,height :  " << pref_width << "  " << pref_height << std::endl;
        }
        ...
        if (vid_err)
        {
            OSG_NOTICE << "VDGetNumberOfInputs(aComponent, &inputs) - ERROR" << std::endl;
        }
        else
        {
            OSG_NOTICE << "Number of inputs        :  " << inputs << std::endl;
        }
        ...
        for (short i = 0; i <= inputs; ++i)
        {
            ...
            if (vid_err)
            {
                OSG_NOTICE << "VDGetInputName(aComponent,(long)i, name) - ERROR" << std::endl;
            }
            else
            {
                OSG_NOTICE << "Name of input   " << i << " :  " << pstr_printable(name) << std::endl;
            }
            ...
            if (vid_err)
            {
                OSG_NOTICE << "VDGetInputFormat(aComponent,(long)i, &input_format) - ERROR" << std::endl;
            }
            else
            {
                if (input_format == compositeIn)
                {
                    OSG_NOTICE << "Format of input :  compositeIn" << std::endl;
                }

                if (input_format == sVideoIn)
                {
                    OSG_NOTICE << "Format of input :  sVideoIn" << std::endl;
                }

                if (input_format == rgbComponentIn)
                {
                    OSG_NOTICE << "Format of input :  rgbComponentIn" << std::endl;
                }

                if (input_format == rgbComponentSyncIn)
                {
                    OSG_NOTICE << "Format of input :  rgbComponentSyncIn" << std::endl;
                }

                if (input_format == yuvComponentIn)
                {
                    OSG_NOTICE << "Format of input :  yuvComponentIn" << std::endl;
                }

                if (input_format == yuvComponentSyncIn)
                {
                    OSG_NOTICE << "Format of input :  yuvComponentSyncIn" << std::endl;
                }

                if (input_format == sdiIn)
                {
                    OSG_NOTICE << "Format of input :  sdiIn" << std::endl;
                }
            }
        }
        ...
        if (vid_err)
        {
            OSG_NOTICE << "VDGetInput(aComponent, &active_input) - ERROR" << std::endl;
        }
        else
        {
            OSG_NOTICE << "Currently active input :  " << active_input << std::endl;
        }
    }
}
