...
void Window::EmbeddedWindow::unparented(Window*)
{
    if (_window.valid())
    {
        ...
        if (_parent)
            _parent->removeChild(_window.get());
    }
}
...
void Window::update()
{
    ...
    {
        if (_vAnchor == VA_TOP)
            y = _wm->getHeight() - _height.current;
        else if (_vAnchor == VA_CENTER)
            y = osg::round(_wm->getHeight() / 2.0f);
        else if (_vAnchor == VA_BOTTOM)
            y = 0.0f;

        if (_hAnchor == HA_LEFT)
            x = 0.0f;
        else if (_hAnchor == HA_CENTER)
            x = osg::round((_wm->getWidth() - _width.current) / 2.0f);
        else if (_hAnchor == HA_RIGHT)
            x = _wm->getWidth() - _width.current + _visibleArea[2];
...
void Window::_setWidthAndHeight()
{
    ...
    if (_width.current < 0.0f)
        _setWidthAndHeightUnknownSizeError("current width", _width.current);

    if (_width.minimum < 0.0f)
        _setWidthAndHeightUnknownSizeError("minimum width", _width.minimum);

    if (_height.current < 0.0f)
        _setWidthAndHeightUnknownSizeError("current height", _height.current);

    if (_height.minimum < 0.0f)
        _setWidthAndHeightUnknownSizeError("minimum height", _height.minimum);

    if (hasDecimal(_width.current))
        _setWidthAndHeightNotPAError("current width", _width.current);

    if (hasDecimal(_width.minimum))
        _setWidthAndHeightNotPAError("minimum width", _width.minimum);

    if (hasDecimal(_height.current))
        _setWidthAndHeightNotPAError("current height", _height.current);

    if (hasDecimal(_height.minimum))
        _setWidthAndHeightNotPAError("minimum height", _height.minimum);
}
...
bool Window::_setWidget(Widget *widget, int index)
{
    ...
    {
        if (_objects[index].valid())
            _removeFromGeode(_objects[index].get());
...
Widget* Window::_getBackground() const
{
    ...
    // lol...
    if (geode)
        return dynamic_cast<Widget*>(const_cast<osg::Drawable*>(geode->getDrawable(0)));
...
void Window::_positionWidget(Widget *widget, point_type width, point_type height)
{
    ...
    if (widget->getAlignHorizontal() == Widget::HA_LEFT)
        widget->addX(pl);
    ...
    if (widget->getAlignVertical() == Widget::VA_BOTTOM)
        widget->addY(height - h - pt);

    else if (widget->getAlignVertical() == Widget::VA_TOP)
        widget->addY(pb);
...
bool Window::setNextFocusable()
{
    ...
    for (unsigned int i = 0; w != focusList.end(); w++, i++)
        if (*w == _focused)
        {
            focusedIndex = i;
    ...
    if (focusedIndex < focusList.size() - 1)
        _setFocused((++w)->get());
...
unsigned int Window::addDrawableAndGetIndex(osg::Drawable *drawable)
{
    ...
    if (geode->addDrawable(drawable))
        return geode->getDrawableIndex(drawable);
