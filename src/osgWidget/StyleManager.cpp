...
{
    if (applyStyle(static_cast<Window*>(box), r))
        return true;
...
Widget::Layer Style::strToLayer(const std::string&layer)
{
    ...
    if (l == "top")
        return Widget::LAYER_TOP;

    else if (l == "high")
        return Widget::LAYER_HIGH;

    else if (l == "middle")
        return Widget::LAYER_MIDDLE;

    else if (l == "low")
        return Widget::LAYER_LOW;

    else if (l == "bg")
        return Widget::LAYER_BG;
...
Widget::VerticalAlignment Style::strToVAlign(const std::string&valign)
{
    ...
    if (va == "center")
        ...
        return Widget::VA_TOP;

    else if (va == "bottom")
        return Widget::VA_BOTTOM;
...
Widget::HorizontalAlignment Style::strToHAlign(const std::string&halign)
{
    ...
    if (ha == "center")
        ...
        return Widget::HA_LEFT;

    else if (ha == "right")
        return Widget::HA_RIGHT;
...
bool StyleManager::_applyStyleToObject(osg::Object *obj, const std::string&style)
{
    ...
    if (!std::string("Widget").compare(c))
        return _coerceAndApply<Widget>(
            obj,
            style,
            c
            );

    else if (!std::string("Label").compare(c))
        return _coerceAndApply<Label>(
            obj,
            style,
            c
            );

    else if (!std::string("Box").compare(c))
        return _coerceAndApply<Box>(
            obj,
            style,
            c
            );
    else if (!std::string("Canvas").compare(c))
        return _coerceAndApply<Canvas>(
            obj,
            style,
            c
            );
    else if (!std::string("Window").compare(c))
        return _coerceAndApply<Window>(
            obj,
            style,
            c
            );
    else if (!std::string("Input").compare(c))
        return _coerceAndApply<Input>(
            obj,
            style,
            c
            );
    else if (!std::string("Corner").compare(c))
        return _coerceAndApply<Frame::Corner>(
            obj,
            style,
            c
            );
    else if (!std::string("Border").compare(c))
        return _coerceAndApply<Frame::Border>(
            obj,
            style,
            c
            );
    else if (!std::string("EmbeddedWindow").compare(c))
        return _coerceAndApply<Window::EmbeddedWindow>(
            obj,
            style,
            c
            );
