...
osg::Image* Widget::_getImage() const
{
    ...
    if (texture)
        return const_cast<osg::Image*>(texture->getImage(0));
...
void Widget::setDimensions(point_type x, point_type y, point_type w, point_type h, point_type z)
{
    ...
    if (_coordMode == CM_ABSOLUTE)
    {
        ...
        if (x < 0.0f)
            x = MACRO_WIDGET_X(verts);

        if (y < 0.0f)
            y = MACRO_WIDGET_Y(verts);

        if (w < 0.0f)
            w = MACRO_WIDGET_W(verts);

        if (h < 0.0f)
            h = MACRO_WIDGET_H(verts);
    }
...
{
    if (_coordMode == CM_ABSOLUTE)
        setDimensions(MACRO_WIDGET_X(_verts()) + x);

    else
        setDimensions(_relCoords[0] + x);
}

void Widget::addY(point_type y)
{
    if (_coordMode == CM_ABSOLUTE)
        setDimensions(-1.0f, MACRO_WIDGET_Y(_verts()) + y);

    else
        setDimensions(-1.0f, _relCoords[1] + y);
}

void Widget::addWidth(point_type w)
{
    if (_coordMode == CM_ABSOLUTE)
        setDimensions(-1.0f, -1.0f, MACRO_WIDGET_W(_verts()) + w);

    else
        setDimensions(-1.0f, -1.0f, _relCoords[2] + w);
}
...
void Widget::addHeight(point_type h)
{
    if (_coordMode == CM_ABSOLUTE)
        ...
        setDimensions(-1.0f, -1.0f, -1.0f, _relCoords[3] + h);
}
...
void Widget::addOrigin(point_type x, point_type y)
{
    if (_coordMode == CM_ABSOLUTE)
    {
        ...
        setDimensions(_relCoords[0] + x, _relCoords[1] + y);
}
...
void Widget::addSize(point_type w, point_type h)
{
    if (_coordMode == CM_ABSOLUTE)
    {
        ...
        setDimensions(-1.0f, -1.0f, _relCoords[2] + w, _relCoords[3] + h);
}
