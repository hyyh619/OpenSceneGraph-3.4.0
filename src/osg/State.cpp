...
{
    if (_checkGLErrors == ONCE_PER_ATTRIBUTE)
        checkGLErrors("start of State::apply(StateSet*)");
    ...
    if (_checkGLErrors == ONCE_PER_ATTRIBUTE)
        checkGLErrors("end of State::apply(StateSet*)");
}

void State::apply()
{
    if (_checkGLErrors == ONCE_PER_ATTRIBUTE)
        checkGLErrors("start of State::apply()");
    ...
    if (_checkGLErrors == ONCE_PER_ATTRIBUTE)
        checkGLErrors("end of State::apply()");
}
...
void State::applyDisablingOfVertexAttributes()
{
    ...
    if (!_useVertexAttributeAliasing)
    {
        if (_vertexArray._lazy_disable)
            disableVertexPointer();

        if (_normalArray._lazy_disable)
            disableNormalPointer();

        if (_colorArray._lazy_disable)
            disableColorPointer();

        if (_secondaryColorArray._lazy_disable)
            disableSecondaryColorPointer();

        if (_fogArray._lazy_disable)
            disableFogCoordPointer();

        for (unsigned int i = 0; i < _texCoordArrayList.size(); ++i)
        {
            if (_texCoordArrayList[i]._lazy_disable)
                disableTexCoordPointer(i);
        }
    }
...
void State::AttributeStack::print(std::ostream &fout) const
{
    ...
    if (last_applied_attribute)
        fout << ", " << last_applied_attribute->className() << ", " << last_applied_attribute->getName() << std::endl;

    fout << "    last_applied_shadercomponent = " << last_applied_shadercomponent << std::endl;
    if (last_applied_shadercomponent)
        fout << ", " << last_applied_shadercomponent->className() << ", " << last_applied_shadercomponent->getName() << std::endl;
...
std::string State::getDefineString(const osg::ShaderDefines &shaderDefines)
{
    ...
    {
        if ((*sd_itr) < cd_itr->first)
            ++sd_itr;
        else if (cd_itr->first < (*sd_itr))
            ++cd_itr;
        else
        {
...
bool State::supportsShaderRequirements(const osg::ShaderDefines &shaderRequirements)
{
    ...
    for (ShaderDefines::const_iterator sr_itr = shaderRequirements.begin();
         ...
         ++sr_itr)
    {
        if (currentDefines.find(*sr_itr) == currentDefines.end())
            return false;
    }
