...
void Texture::TextureObjectManager::releaseTextureObject(Texture::TextureObject *to)
{
    if (to->_set)
        ...
        OSG_NOTICE << "TextureObjectManager::releaseTextureObject(Texture::TextureObject* to) Not implemented yet" << std::endl;
}
...
void Texture::TextureObjectManager::recomputeStats(std::ostream &out) const
{
    ...
    if (currentSize != _currTexturePoolSize)
        out << "   WARNING: _currTexturePoolSize(" << _currTexturePoolSize << ") != currentSize, delta = " << int(_currTexturePoolSize) - int(currentSize) << std::endl;
}
...
int Texture::compareTextureObjects(const Texture &rhs) const
{
    ...
    if (rhs._textureObjectBuffer.size() < _textureObjectBuffer.size())
        return 1;

    for (unsigned int i = 0; i < _textureObjectBuffer.size(); ++i)
    {
        if (_textureObjectBuffer[i] < rhs._textureObjectBuffer[i])
            return -1;
        else if (rhs._textureObjectBuffer[i] < _textureObjectBuffer[i])
            return 1;
    }
...
void Texture::computeRequiredTextureDimensions(State &state, const osg::Image &image, GLsizei &inwidth, GLsizei &inheight, GLsizei &numMipmapLevels) const
{
    ...
    if (width > extensions->maxTextureSize)
        width = extensions->maxTextureSize;

    if (height > extensions->maxTextureSize)
        height = extensions->maxTextureSize;
...
void Texture::applyTexImage2D_load(State &state, GLenum target, const Image *image, GLsizei inwidth, GLsizei inheight, GLsizei numMipmapLevels) const
{
    ...
    if (needImageRescale)
    {
        ...
        {
            OSG_NOTICE << "Scaling image '" << image->getFileName() << "' from (" << image->s() << "," << image->t() << ") to (" << inwidth << "," << inheight << ")" << std::endl;
        }
        else
        {
            OSG_NOTICE << "Scaling image from (" << image->s() << "," << image->t() << ") to (" << inwidth << "," << inheight << ")" << std::endl;
        }
