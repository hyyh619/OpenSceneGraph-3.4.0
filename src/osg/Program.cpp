...
int Program::compare(const osg::StateAttribute&sa) const
{
    ...
    if (rhs._shaderList.size() < _shaderList.size())
        return 1;
    ...
    if (_geometryVerticesOut < rhs._geometryVerticesOut)
        return -1;

    if (rhs._geometryVerticesOut < _geometryVerticesOut)
        return 1;

    if (_geometryInputType < rhs._geometryInputType)
        return -1;

    if (rhs._geometryInputType < _geometryInputType)
        return 1;

    if (_geometryOutputType < rhs._geometryOutputType)
        return -1;

    if (rhs._geometryOutputType < _geometryOutputType)
        return 1;

    if (_numGroupsX < rhs._numGroupsX)
        return -1;

    if (rhs._numGroupsX < _numGroupsX)
        return 1;

    if (_numGroupsY < rhs._numGroupsY)
        return -1;

    if (rhs._numGroupsY < _numGroupsY)
        return 1;

    if (_numGroupsZ < rhs._numGroupsZ)
        return -1;

    if (rhs._numGroupsZ < _numGroupsZ)
        return 1;

    if (_feedbackout < rhs._feedbackout)
        return -1;

    if (_feedbackmode < rhs._feedbackmode)
        return -1;
...
void Program::dirtyProgram()
{
    ...
    {
        if (_pcpList[cxt].valid())
            _pcpList[cxt]->requestLink();
    }
...
bool Program::addShader(Shader *shader)
{
    ...
    {
        if (_pcpList[cxt].valid())
            _pcpList[cxt]->addShaderToAttach(shader);
    }
...
bool Program::removeShader(Shader *shader)
{
         ...
         ++itr)
    {
        if (shader == itr->get())
        {
            ...
            {
                if (_pcpList[cxt].valid())
                    _pcpList[cxt]->addShaderToDetach(shader);
            }
...
void Program::PerContextProgram::linkProgram(osg::State&state)
{
    ...
    if (!_loadedBinary)
    {
        ...
        for (unsigned int i = 0; i < _shadersToDetach.size(); ++i)
        {
            ...
            if (pcs)
                _extensions->glDetachShader(_glProgramHandle, pcs->getHandle());
        }
    }
    ...
    if (!_loadedBinary)
    {
        ...
        for (unsigned int i = 0; i < _shadersToAttach.size(); ++i)
        {
            ...
            if (pcs)
                _extensions->glAttachShader(_glProgramHandle, pcs->getHandle());
        }
    }
