...
void StateSet::computeDataVariance()
{
         ...
         ++itr)
    {
        ...
        if (itr->second.first->getDataVariance() == DYNAMIC)
            dynamic = true;
    }
         ...
         ++uitr)
    {
        ...
        if (uitr->second.first->getDataVariance() == DYNAMIC)
            dynamic = true;
    }
...
int StateSet::compare(const StateSet &rhs, bool compareAttributeContents) const
{
    ...
    if (_textureModeList.size() < rhs._textureModeList.size())
        return -1;

    if (_textureModeList.size() > rhs._textureModeList.size())
        return 1;

    if (_attributeList.size() < rhs._attributeList.size())
        return -1;

    if (_attributeList.size() > rhs._attributeList.size())
        return 1;

    if (_modeList.size() < rhs._modeList.size())
        return -1;

    if (_modeList.size() > rhs._modeList.size())
        return 1;

    if (_uniformList.size() < rhs._uniformList.size())
        return -1;

    if (_uniformList.size() > rhs._uniformList.size())
        return 1;

    if (_defineList.size() < rhs._defineList.size())
        return -1;
    ...
    if (_binMode < rhs._binMode)
        return -1;
    else if (_binMode > rhs._binMode)
        return 1;

    if (_binMode != INHERIT_RENDERBIN_DETAILS)
    {
        if (_binNum < rhs._binNum)
            return -1;
        else if (_binNum > rhs._binNum)
            return 1;

        if (_binName < rhs._binName)
            return -1;
        else if (_binName > rhs._binName)
            return 1;
    }
    ...
    for (unsigned int ai = 0; ai < _textureAttributeList.size(); ++ai)
    {
        ...
        if (compareAttributeContents)
        {
            ...
            {
                if (lhs_attr_itr->first < rhs_attr_itr->first)
                    return -1;
                else if (rhs_attr_itr->first < lhs_attr_itr->first)
                    return 1;

                if (*(lhs_attr_itr->second.first) < *(rhs_attr_itr->second.first))
                    return -1;
                else if (*(rhs_attr_itr->second.first) < *(lhs_attr_itr->second.first))
                    return 1;

                if (lhs_attr_itr->second.second < rhs_attr_itr->second.second)
                    return -1;
                else if (rhs_attr_itr->second.second < lhs_attr_itr->second.second)
                    return 1;
            ...
            {
                if (rhs_attr_itr != rhs_attributeList.end())
                    return -1;
            }
            else if (rhs_attr_itr == rhs_attributeList.end())
                return 1;
        }
    ...
    if (compareAttributeContents)
    {
        ...
        {
            if (rhs_attr_itr != rhs._attributeList.end())
                return -1;
        }
        else if (rhs_attr_itr == rhs._attributeList.end())
            return 1;
    }
    else // just compare pointers.
    {
    ...
    for (unsigned int ti = 0; ti < _textureModeList.size(); ++ti)
    {
        ...
        {
            if (lhs_mode_itr->first < rhs_mode_itr->first)
                return -1;
            else if (rhs_mode_itr->first < lhs_mode_itr->first)
                return 1;

            if (lhs_mode_itr->second < rhs_mode_itr->second)
                return -1;
            else if (rhs_mode_itr->second < lhs_mode_itr->second)
                return 1;
        ...
        {
            if (rhs_mode_itr != rhs_modeList.end())
                return -1;
        }
        else if (rhs_mode_itr == rhs_modeList.end())
            return 1;
    }
    ...
    {
        if (rhs_mode_itr != rhs._modeList.end())
            return -1;
    }
    else if (rhs_mode_itr == rhs._modeList.end())
        return 1;
    ...
    {
        if (lhs_uniform_itr->first < rhs_uniform_itr->first)
            return -1;
        else if (rhs_uniform_itr->first < lhs_uniform_itr->first)
            return 1;

        if (*lhs_uniform_itr->second.first < *rhs_uniform_itr->second.first)
            return -1;
        else if (*rhs_uniform_itr->second.first < *lhs_uniform_itr->second.first)
            return 1;

        if (lhs_uniform_itr->second.second < rhs_uniform_itr->second.second)
            return -1;
        else if (rhs_uniform_itr->second.second < lhs_uniform_itr->second.second)
            return 1;
    ...
    {
        if (rhs_uniform_itr != rhs._uniformList.end())
            return -1;
    }
    else if (rhs_uniform_itr == rhs._uniformList.end())
        return 1;
    ...
    {
        if (lhs_define_itr->first < rhs_define_itr->first)
            return -1;
        else if (rhs_define_itr->first < lhs_define_itr->first)
            return 1;

        if (lhs_define_itr->second.first < rhs_define_itr->second.first)
            return -1;
        else if (rhs_define_itr->second.first < lhs_define_itr->second.first)
            return 1;

        if (lhs_define_itr->second.second < rhs_define_itr->second.second)
            return -1;
        else if (rhs_define_itr->second.second < lhs_define_itr->second.second)
            return 1;
    ...
    {
        if (rhs_define_itr != rhs._defineList.end())
            return -1;
    }
    else if (rhs_define_itr == rhs._defineList.end())
        return 1;
...
int StateSet::compareModes(const ModeList &lhs, const ModeList &rhs)
{
    ...
    {
        if (rhs_mode_itr != rhs.end())
            return -1;
    }
    else if (rhs_mode_itr == rhs.end())
        return 1;
...
int StateSet::compareAttributePtrs(const AttributeList &lhs, const AttributeList &rhs)
{
    ...
    {
        if (rhs_attr_itr != rhs.end())
            return -1;
    }
    else if (rhs_attr_itr == rhs.end())
        return 1;
...
void StateSet::merge(const StateSet &rhs)
{
    ...
    if (_textureModeList.size() < rhs._textureModeList.size())
        _textureModeList.resize(rhs._textureModeList.size());
...
void StateSet::removeDefine(const std::string &defineName)
{
    ...
    if (itr != _defineList.end())
        _defineList.erase(itr);
}
...
{
    if (unit >= _textureAttributeList.size())
        return;
...
bool StateSet::checkValidityOfAssociatedModes(osg::State &state) const
{
    ...
    for (AttributeList::const_iterator itr = _attributeList.begin();
         ...
         ++itr)
    {
        if (!itr->second.first->checkValidityOfAssociatedModes(state))
            modesValid = false;
    }
...
void StateSet::runUpdateCallbacks(osg::NodeVisitor *nv)
{
    ...
    if (_numChildrenRequiringUpdateTraversal != 0)
    {
             ...
             ++itr)
        {
            ...
            if (callback)
                (*callback)(itr->second.first.get(), nv);
        }
             ...
             ++uitr)
        {
            ...
            if (callback)
                (*callback)(uitr->second.first.get(), nv);
        }
    }
}
