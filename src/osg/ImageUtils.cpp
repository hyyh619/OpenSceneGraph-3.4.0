...
struct FindRangeOperator : public CastAndScaleToFloatOperation
{
    ...
    inline void luminance_alpha(float l, float a)
    {
        rgba(l, l, l, a);
    }
...
struct SetToColourOperator
{
    ...
    inline void luminance_alpha(float&l, float&a) const
    {
        l = (_colour.r() + _colour.g() + _colour.b()) * 0.333333; a = _colour.a();
    }
    inline void rgb(float&r, float&g, float&b) const
    {
        r = _colour.r(); g = _colour.g(); b = _colour.b();
    }
    inline void rgba(float&r, float&g, float&b, float&a) const
    {
        r = _colour.r(); g = _colour.g(); b = _colour.b(); a = _colour.a();
    }
                          ...
                          bool resizeToPowerOfTwo)
{
    ...
    {
        while (size_s < max_s && size_s < s_maximumImageSize)
            size_s *= 2;

        while (size_t < max_t && size_t < t_maximumImageSize)
            size_t *= 2;

        while (size_r < total_r && size_r < r_maximumImageSize)
            size_r *= 2;
    }
    else
    {
...
{
    ModulateAlphaByColorOperator(const osg::Vec4&colour) : _colour(colour)
    {
        _lum = _colour.length();
    }
