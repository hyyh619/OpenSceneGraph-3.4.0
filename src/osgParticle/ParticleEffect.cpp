...
{
    if (_useLocalParticleSystem == local)
        return;
...
{
    if (_defaultParticleTemplate.getLifeTime() == duration)
        return;
...
void ParticleEffect::buildEffect()
{
    ...
    if (!emitter || !particleSystem || !program)
        return;
