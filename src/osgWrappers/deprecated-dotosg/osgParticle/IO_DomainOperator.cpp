...
bool DomainOperator_readLocalData(osg::Object &obj, osgDB::Input &fr)
{
    ...
    {
        if (fr[1].getStr())
            typeName = fr[1].getStr();

        fr += 3;

        osgParticle::DomainOperator::Domain::Type type = osgParticle::DomainOperator::Domain::UNDEFINED_DOMAIN;
        if (typeName == "point")
            type = osgParticle::DomainOperator::Domain::POINT_DOMAIN;
        else if (typeName == "line")
            type = osgParticle::DomainOperator::Domain::LINE_DOMAIN;
        else if (typeName == "triangle")
            type = osgParticle::DomainOperator::Domain::TRI_DOMAIN;
        else if (typeName == "rectangle")
            type = osgParticle::DomainOperator::Domain::RECT_DOMAIN;
        else if (typeName == "plane")
            type = osgParticle::DomainOperator::Domain::PLANE_DOMAIN;
        else if (typeName == "sphere")
            type = osgParticle::DomainOperator::Domain::SPHERE_DOMAIN;
        else if (typeName == "box")
            type = osgParticle::DomainOperator::Domain::BOX_DOMAIN;
        else if (typeName == "disk")
            type = osgParticle::DomainOperator::Domain::DISK_DOMAIN;
