...
bool PolygonMode_readLocalData(Object&obj, Input&fr)
{
    ...
    if (fr[0].matchWord("mode"))
    {
        ...
        if (fr[1].matchWord("FRONT"))
            face = PolygonMode::FRONT;
        else if (fr[1].matchWord("BACK"))
            face = PolygonMode::BACK;
        else if (fr[1].matchWord("FRONT_AND_BACK"))
            face = PolygonMode::FRONT_AND_BACK;
        else
            readFace = false;
        ...
        if (readFace)
        {
            ...
            bool              readMode = true;
            if (fr[2].matchWord("POINT"))
                mode = PolygonMode::POINT;
            else if (fr[2].matchWord("LINE"))
                mode = PolygonMode::LINE;
            else if (fr[2].matchWord("FILL"))
