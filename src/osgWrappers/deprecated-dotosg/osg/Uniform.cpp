...
bool Uniform_writeLocalData(const Object&obj, Output&fw)
{
    ...
    if (uniform.getFloatArray())
        Array_writeLocalData(*uniform.getFloatArray(), fw);

    if (uniform.getIntArray())
        Array_writeLocalData(*uniform.getIntArray(), fw);

    if (uniform.getUIntArray())
        Array_writeLocalData(*uniform.getUIntArray(), fw);
