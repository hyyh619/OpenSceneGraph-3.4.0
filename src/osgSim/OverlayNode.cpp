...
Face&createFace()
{
    _faces.push_back(Face()); return _faces.back();
}
         ...
         ++itr)
    {
        ...
        for (unsigned int i = 0; i < face.vertices.size(); ++i)
        {
            ...
            if (va < vb)
                ...
                edgeMap[EdgePair(vb, va)].push_back(&face);
        }
    }
...
{
    if (_camera.valid())
        _camera->setThreadSafeRefUnref(threadSafe);

    if (_texgenNode.valid())
        _texgenNode->setThreadSafeRefUnref(threadSafe);

    if (_overlayStateSet.valid())
        _overlayStateSet->setThreadSafeRefUnref(threadSafe);

    if (_mainSubgraphStateSet.valid())
        _mainSubgraphStateSet->setThreadSafeRefUnref(threadSafe);

    if (_texture.valid())
        _texture->setThreadSafeRefUnref(threadSafe);
}
...
void OverlayNode::OverlayData::resizeGLObjectBuffers(unsigned int maxSize)
{
    ...
    if (_texgenNode.valid())
        _texgenNode->resizeGLObjectBuffers(maxSize);

    if (_overlayStateSet.valid())
        _overlayStateSet->resizeGLObjectBuffers(maxSize);

    if (_mainSubgraphStateSet.valid())
        _mainSubgraphStateSet->resizeGLObjectBuffers(maxSize);
...
void OverlayNode::OverlayData::releaseGLObjects(osg::State *state) const
{
    ...
    if (_texgenNode.valid())
        _texgenNode->releaseGLObjects(state);

    if (_overlayStateSet.valid())
        _overlayStateSet->releaseGLObjects(state);

    if (_mainSubgraphStateSet.valid())
        _mainSubgraphStateSet->releaseGLObjects(state);
...
void OverlayNode::setThreadSafeRefUnref(bool threadSafe)
{
    ...
    if (_overlaySubgraph.valid())
        _overlaySubgraph->setThreadSafeRefUnref(threadSafe);
...
void OverlayNode::resizeGLObjectBuffers(unsigned int maxSize)
{
    ...
    if (_overlaySubgraph.valid())
        _overlaySubgraph->resizeGLObjectBuffers(maxSize);
...
void OverlayNode::releaseGLObjects(osg::State *state) const
{
    ...
    if (_overlaySubgraph.valid())
        _overlaySubgraph->releaseGLObjects(state);
...
{
    if (_overlayTechnique == technique)
        return;
...
OverlayNode::OverlayData* OverlayNode::getOverlayData(osgUtil::CullVisitor *cv)
{
    ...
    if (!overlayData->_camera || overlayData->_camera->getRenderTargetImplementation() != _renderTargetImpl)
    {
        ...
        if (_overlaySubgraph.valid())
            overlayData->_camera->addChild(_overlaySubgraph.get());
    }
    ...
    if (!overlayData->_y0)
        overlayData->_y0 = new osg::Uniform("y0", 0.0f);

    if (!overlayData->_lightingEnabled)
        overlayData->_lightingEnabled = new osg::Uniform("lightingEnabled", true);
...
void OverlayNode::traverse_VIEW_DEPENDENT_WITH_ORTHOGRAPHIC_OVERLAY(osg::NodeVisitor&nv)
{
    ...
    if (_overlaySubgraph.valid())
    {
        ...
        for (i = 0; i < corners.size(); ++i)
        {
            ...
            if (distanceEye < min_distanceEye)
                min_distanceEye = distanceEye;

            if (distanceEye > max_distanceEye)
                max_distanceEye = distanceEye;
            ...
            if (distance_up < min_up)
                min_up = distance_up;

            if (distance_up > max_up)
                max_up = distance_up;
        }
...
//            double original_width = max_side-min_side;
            ...
            for (i = 0; i < projectedVertices.size(); ++i)
            {
                ...
                if (side_over_up > max_side_over_up)
                    max_side_over_up = side_over_up;
            }
...
void OverlayNode::setOverlayTextureSizeHint(unsigned int size)
{
         ...
         ++itr)
    {
        if (itr->second->_texture.valid())
            itr->second->_texture->setTextureSize(_textureSizeHint, _textureSizeHint);

        if (itr->second->_camera.valid())
            itr->second->_camera->setViewport(0, 0, _textureSizeHint, _textureSizeHint);
    }
...
void OverlayNode::updateMainSubgraphStateSet()
{
         ...
         ++itr)
    {
        ...
        if (texgenNode)
            texgenNode->setTextureUnit(_textureUnit);
