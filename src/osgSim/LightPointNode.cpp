...
void LightPointNode::traverse(osg::NodeVisitor &nv)
{
    ...
    if (cv /*&& !cv->isCulled(_bbox)*/)
    {
             ...
             ++itr)
        {
            ...
            {
                if (dv.length2() > _maxVisibleDistance2)
                    continue;
            ...
            if (pixelSize < _minPixelSize)
                pixelSize = _minPixelSize;
