...
void DirectionalSector::setFadeAngle(float angle)
{
    ...
    if (ang > osg::PI)
        ...
        _cosHorizFadeAngle = cos(ang);

    ang = acos(_cosVertAngle) + angle;
    if (ang > osg::PI)
        _cosVertFadeAngle = -1.0;
    else
        _cosVertFadeAngle = cos(ang);
}
