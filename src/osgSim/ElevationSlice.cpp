...
struct Segment
{
    ...
    Classification compare(const Segment &rhs) const
    {
        ...
        {
            if (fabs(_p2->height - rhs._p1->height) < epsilon)
                return JOINED;
        }
        ...
        if (rhs._p2->distance < _p1->distance || _p2->distance < rhs._p1->distance)
            return SEPARATE;
        ...
        if (rhs_p1_inside && rhs_p2_inside)
            return ENCLOSING;
        ...
        if (rhs_p1_inside || rhs_p2_inside || p1_inside || p2_inside)
            return OVERLAPPING;
...
void ElevationSlice::computeIntersections(osg::Node *scene, osg::Node::NodeMask traversalMask)
{
    ...
    if (!intersections.empty())
    {
        ...
        while (numOverlapping > 0)
        {
            ...
            if (previousNumOverlapping == numOverlapping)
                break;
        }
