...
int ShapeAttribute::compare(const osgSim::ShapeAttribute&sa) const
{
    ...
    case STRING:
    {
        if (_string < sa._string)
            return -1;

        if (sa._string < _string)
            return 1;
    }

    case DOUBLE:
    {
        if (_double < sa._double)
            return -1;

        if (sa._double < _double)
            return 1;
    }
    ...
    default:
    {
        if (_integer < sa._integer)
            return -1;

        if (sa._integer < _integer)
            return 1;
    }
    }
