...
protected:
    ...
    ActivateTransparencyOnType&operator =(const ActivateTransparencyOnType&)
    {
        return *this;
    }
};
...
protected:
    ...
    DeactivateTransparencyOnType&operator =(const DeactivateTransparencyOnType&)
    {
        return *this;
    }
};
...
void SphereSegment::setSurfaceColor(const osg::Vec4 &c)
{
    ...
    if (c.w() != 1.0)
        std::for_each(_children.begin(), _children.end(), ActivateTransparencyOnType(typeid(Surface)));
    else
...
void SphereSegment::setSpokeColor(const osg::Vec4 &c)
{
    ...
    if (c.w() != 1.0)
        std::for_each(_children.begin(), _children.end(), ActivateTransparencyOnType(typeid(Spoke)));
    else
...
void SphereSegment::setEdgeLineColor(const osg::Vec4 &c)
{
    ...
    if (c.w() != 1.0)
        std::for_each(_children.begin(), _children.end(), ActivateTransparencyOnType(typeid(EdgeLine)));
    else
...
void SphereSegment::setSideColor(const osg::Vec4 &c)
{
    ...
    if (c.w() != 1.0)
        std::for_each(_children.begin(), _children.end(), ActivateTransparencyOnType(typeid(Side)));
    else
...
protected:
    ...
    SortFunctor&operator =(const SortFunctor&)
    {
        return *this;
    }
};
...
struct TriangleIntersectOperator
{
    ...
    struct Edge : public osg::Referenced
    {
        ...
        void removeFromToTraverseList(Triangle *tri)
        {
            ...
            if (itr != _toTraverse.end())
                _toTraverse.erase(itr);
        }


        bool completlyOutside() const
        {
            return _p1Outside && _p2Outside;
        }
    ...
    struct Triangle : public osg::Referenced
    {
        ...
        Edge* oppositeActiveEdge(Edge *edge)
        {
            ...
            if (edge != _e1 && _e1 && _e1->_intersectionType != Edge::NO_INTERSECTION)
                return _e1;

            if (edge != _e2 && _e2 && _e2->_intersectionType != Edge::NO_INTERSECTION)
                return _e2;

            if (edge != _e3 && _e3 && _e3->_intersectionType != Edge::NO_INTERSECTION)
                return _e3;
    ...
    struct Region
    {
        ...
        void classify(const osg::Vec3 &vertex, double radius2, double azimMin, double azimMax, double elevMin, double elevMax)
        {
            ...
            if (rad2 > radius2)
                _radiusSurface = OUTSIDE;
            else if (rad2 < radius2)
                _radiusSurface = INSIDE;
            else
                _radiusSurface = INTERSECTS;

            // bottom surface
            if (elevation < elevMin)
                _bottomSurface = OUTSIDE;
            else if (elevation > elevMin)
                _bottomSurface = INSIDE;
            else
                _bottomSurface = INTERSECTS;

            // top surface
            if (elevation > elevMax)
                _topSurface = OUTSIDE;
            else if (elevation < elevMax)
                _topSurface = INSIDE;
            else
                _topSurface = INTERSECTS;


            double dot_alphaMin = cos(azimMin) * vertex.x() - sin(azimMin) * vertex.y();
            if (dot_alphaMin < 0.0)
                _leftSurface = OUTSIDE;
            else if (dot_alphaMin > 0.0)
                _leftSurface = INSIDE;
            else
    ...
    struct RegionCounter
    {
        ...
        void add(const Region &region)
        {
            ...
            if (region._radiusSurface == Region::OUTSIDE)
                ++_outside_radiusSurface;

            if (region._radiusSurface == Region::INSIDE)
                ++_inside_radiusSurface;

            if (region._radiusSurface == Region::INTERSECTS)
                ++_intersects_radiusSurface;

            if (region._leftRightSurfaces == Region::OUTSIDE)
                ++_outside_leftRightSurfaces;

            if (region._leftRightSurfaces == Region::INSIDE)
                ++_inside_leftRightSurfaces;

            if (region._leftRightSurfaces == Region::INTERSECTS)
                ++_intersects_leftRightSurfaces;

            if (region._leftSurface == Region::OUTSIDE)
                ++_outside_leftSurface;

            if (region._leftSurface == Region::INSIDE)
                ++_inside_leftSurface;

            if (region._leftSurface == Region::INTERSECTS)
                ++_intersects_leftSurface;

            if (region._rightSurface == Region::OUTSIDE)
                ++_outside_rightSurface;

            if (region._rightSurface == Region::INSIDE)
                ++_inside_rightSurface;

            if (region._rightSurface == Region::INTERSECTS)
                ++_intersects_rightSurface;

            if (region._bottomSurface == Region::OUTSIDE)
                ++_outside_bottomSurface;

            if (region._bottomSurface == Region::INSIDE)
                ++_inside_bottomSurface;

            if (region._bottomSurface == Region::INTERSECTS)
                ++_intersects_bottomSurface;

            if (region._topSurface == Region::OUTSIDE)
                ++_outside_topSurface;

            if (region._topSurface == Region::INSIDE)
                ++_inside_topSurface;

            if (region._topSurface == Region::INTERSECTS)
                ++_intersects_topSurface;
        }
        ...
        Region::Classification overallClassification() const
        {
            ...
            if (_outside_radiusSurface == _numVertices ||
                ...
                _outside_bottomSurface == _numVertices)
            ...
            if (_inside_radiusSurface == _numVertices &&
                ...
                _inside_bottomSurface == _numVertices)
        ...
        bool intersecting(SurfaceType surfaceType) const
        {
            ...
            if ((surfaceType != RADIUS_SURFACE && _outside_radiusSurface != 0) ||
                ...
                (surfaceType != BOTTOM_SURFACE && _outside_bottomSurface != 0))
                return false;
            ...
            if ((surfaceType != RADIUS_SURFACE && _inside_radiusSurface != 0) &&
                ...
                (surfaceType != BOTTOM_SURFACE && _inside_bottomSurface != 0))
                return false;
        ...
        int numberOfIntersectingSurfaces() const
        {
            ...
            if (_outside_radiusSurface != _numVertices && _inside_radiusSurface != _numVertices)
                ++sidesThatIntersect;

            if (_outside_leftSurface != _numVertices && _inside_leftSurface != _numVertices)
                ++sidesThatIntersect;

            if (_outside_rightSurface != _numVertices && _inside_rightSurface != _numVertices)
                ++sidesThatIntersect;

            if (_outside_topSurface != _numVertices && _inside_topSurface != _numVertices)
                ++sidesThatIntersect;

            if (_outside_bottomSurface != _numVertices && _inside_bottomSurface != _numVertices)
                ++sidesThatIntersect;
    ...
    void removeDuplicateVertices()
    {
        ...
        if (_candidateVertexIndices.size() < 2)
            return;
    ...
    void buildEdges()
    {
             ...
             ++eitr)
        {
            ...
            if (numConnections == 0)
                ...
                ++numSingleConnections;
            else if (numConnections == 2)
                ++numDoubleConnections;
            else
                ++numMultiConnections;
        }
    ...
    SphereSegment::LineList connectIntersections(EdgeList &hitEdges)
    {
             ...
             ++hitr)
        {
                 ...
                 ++titr)
            {
                ...
                if (tri->_e1 && tri->_e1->_intersectionType != Edge::NO_INTERSECTION)
                    ++numActiveEdges;

                if (tri->_e2 && tri->_e2->_intersectionType != Edge::NO_INTERSECTION)
                    ++numActiveEdges;

                if (tri->_e3 && tri->_e3->_intersectionType != Edge::NO_INTERSECTION)
                    ++numActiveEdges;
    ...
    void trim(SphereSegment::LineList &lineList, osg::Vec3Array *sourceLine, I intersector)
    {
        ...
        while (first < sourceLine->size())
        {
            ...
            {
                if (intersector.distance((*sourceLine)[first] - _centre) >= 0.0)
                    break;
            }
            ...
            {
                if (intersector.distance((*sourceLine)[last] - _centre) < 0.0)
                    break;
            }
    ...
    void trim(SphereSegment::LineList &lineList, osg::Vec3Array *sourceLine, I intersector1, I intersector2)
    {
        ...
        while (first < sourceLine->size())
        {
            ...
            for (; first < sourceLine->size(); ++first)
            {
                ...
                if (intersector1.distance(v) >= 0.0 || intersector2.distance(v) >= 0.0)
                    break;
            }
...
protected:
    ...
    AzimPlaneIntersector&operator =(const AzimPlaneIntersector&)
    {
        return *this;
    }
};
...
protected:
    ...
    ElevationIntersector&operator =(const ElevationIntersector&)
    {
        return *this;
    }
};
...
struct RadiusIntersector
{
    ...
    inline bool operator()(TriangleIntersectOperator::Edge *edge)
    {
        ...
        // if both points inside then discard
        if (radius1 < _tif._radius && radius2 < _tif._radius)
            return false;
...
protected:
    ...
    RadiusIntersector&operator =(const RadiusIntersector&)
    {
        return *this;
    }
};
