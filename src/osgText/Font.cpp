    ...
    _numCurveSamples(10)
{
    ...
    if ((ptr = getenv("OSG_MAX_TEXTURE_SIZE")) != 0)
    {
        ...
        if (osg_max_size < _textureWidthHint)
            _textureWidthHint = osg_max_size;

        if (osg_max_size < _textureHeightHint)
            _textureHeightHint = osg_max_size;
    }
}
...
{
    if (_implementation.valid())
        return _implementation->getFileName();
...
Glyph* Font::getGlyph(const FontResolution&fontRes, unsigned int charcode)
{
    ...
    if (_implementation->supportsMultipleFontResolutions())
        fontResUsed = fontRes;
...
void Font::setThreadSafeRefUnref(bool threadSafe)
{
    ...
    if (_texenv.valid())
        _texenv->setThreadSafeRefUnref(threadSafe);
...
{
    if (_implementation.valid())
        return _implementation->getKerning(leftcharcode, rightcharcode, kerningType);
    else
...
{
    if (_implementation.valid())
        return _implementation->hasVertical();
    else
        return false;
}
