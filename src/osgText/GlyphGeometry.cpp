...
bool shorter(float original_thickness, float new_thickness) const
{
...
bool shorten(float &original_thickness, float new_thickness)
{
    ...
    {
        original_thickness = new_thickness; return true;
    }
    else
        return false;
}
...
void addBoundaryToGeometry(osg::Geometry *geometry, float targetThickness, const std::string &faceName, const std::string &bevelName)
{
    ...
    if (geometry->getVertexArray() == 0)
        geometry->setVertexArray(new osg::Vec3Array(*_vertices));
        ...
        if (elements && elements->size() > 2)
        {
            ...
            for (int i = 0; i < num_indices - 1; ++i)
            {
                ...
                if (vi_curr >= static_cast<int>(remappedIndices.size()))
                    remappedIndices.resize(vi_curr + 1, -1);
