...
{
    if (!_style)
        return _characterHeight * 0.1f;
    else
        return _characterHeight * _style->getThicknessRatio();
}
...
String::iterator Text3D::computeLastCharacterOnLine(osg::Vec2 &cursor, String::iterator first, String::iterator last)
{
    ...
    for (bool outOfSpace = false; lastChar != last; ++lastChar)
    {
        ...
        if (glyph)
        {
            ...
            case LEFT_TO_RIGHT:
            {
                if (maximumWidth > 0.0f && local.x() + bb.xMax() > maximumWidth)
                    outOfSpace = true;

                if (maximumHeight > 0.0f && local.y() < -maximumHeight)
                    outOfSpace = true;
            ...
            case RIGHT_TO_LEFT:
            {
                if (maximumWidth > 0.0f && local.x() + bb.xMin() < -maximumWidth)
                    outOfSpace = true;
...
void Text3D::renderPerGlyph(osg::State &state) const
{
    ...
    if (wallStateSet == 0)
        wallStateSet = frontStateSet;
    else if (wallStateSet->getAttribute(osg::StateAttribute::MATERIAL) != 0)
        applyMainColor = true;

    if (backStateSet == 0)
        backStateSet = frontStateSet;
    else if (backStateSet->getAttribute(osg::StateAttribute::MATERIAL) != 0)
        applyMainColor = true;
    ...
    for (itLine = _textRenderInfo.begin(); itLine != endLine; ++itLine)
    {
        ...
        for (it = itLine->begin(); it != end; ++it)
        {
            ...
            if (wallStateSet != frontStateSet)
                state.apply(wallStateSet);
...
void Text3D::setThreadSafeRefUnref(bool threadSafe)
{
    ...
    if (_font.valid())
        _font->setThreadSafeRefUnref(threadSafe);
}
