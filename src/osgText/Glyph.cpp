...
void GlyphTexture::apply(osg::State &state) const
{
    ...
    if (!glyphsWereSubloading.empty() || newTextureObject)
    {
        ...
        #if defined(OSG_GLES2_AVAILABLE)
        if (requiresGenerateMipmapCall)
            subloadAllGlyphsTogether = true;
        #endif
...
const osg::Vec2&Glyph::getHorizontalBearing() const
{
    return _horizontalBearing;
}

void Glyph::setHorizontalAdvance(float advance)
{
    _horizontalAdvance = advance;
}
float Glyph::getHorizontalAdvance() const
{
...
const osg::Vec2&Glyph::getVerticalBearing() const
{
    return _verticalBearing;
}

void Glyph::setVerticalAdvance(float advance)
{
    _verticalAdvance = advance;
}
float Glyph::getVerticalAdvance() const
{
    return _verticalAdvance;
}

void Glyph::setTexture(GlyphTexture *texture)
{
    _texture = texture;
}
GlyphTexture* Glyph::getTexture()
{
    return _texture;
}
const GlyphTexture* Glyph::getTexture() const
{
    return _texture;
}

void Glyph::setTexturePosition(int posX, int posY)
{
    _texturePosX = posX; _texturePosY = posY;
}
int Glyph::getTexturePositionX() const
{
    return _texturePosX;
}
int Glyph::getTexturePositionY() const
{
    return _texturePosY;
}
...
const osg::Vec2&Glyph::getMinTexCoord() const
{
    return _minTexCoord;
}
...
const osg::Vec2&Glyph::getMaxTexCoord() const
{
    return _maxTexCoord;
}
...
void Glyph::subload() const
{
    ...
    if (errorNo != GL_NO_ERROR)
    {
        ...
        if (msg)
        {
            OSG_WARN << "before Glyph::subload(): detected OpenGL error: " << msg << std::endl;
        }
        else
        {
    ...
    if (errorNo != GL_NO_ERROR)
    {
        ...
        if (msg)
        {
            OSG_WARN << "after Glyph::subload() : detected OpenGL error: " << msg << std::endl;
        }
        else
        {
...
{
    if (_geode.valid())
        _geode->setThreadSafeRefUnref(threadSafe);
}
...
void GlyphGeometry::setup(const Glyph3D *glyph, const Style *style)
{
    ...
    if (shellGeometry.valid())
        _geode->addDrawable(shellGeometry.get());
         ...
         ++itr)
    {
        ...
        if (prim->getName() == "front")
            _frontPrimitiveSetList.push_back(prim);
        else if (prim->getName() == "back")
            _backPrimitiveSetList.push_back(prim);
        else if (prim->getName() == "wall")
            _wallPrimitiveSetList.push_back(prim);
    }
}
