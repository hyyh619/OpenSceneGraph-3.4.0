...
void setFileCache(osgDB::FileCache *fileCache)
{
    ...
    if (intersects())
    {
        for (unsigned int i = 0; i < plod.getNumFileNames(); ++i)
        {
            ...
            if (!plod.getFileName(i).empty())
            {
                ...
                if (!s_ExitApplication && node.valid())
                    node->accept(*this);
            }
        }
    }
...
void updateBound(osg::Vec3Array &vertices)
{
         ...
         ++itr)
    {
        ...
        if (em)
            convertXYZToLatLongHeight(em, v);
...
bool intersects()
{
    ...
    for (ExtentsList::iterator itr = _extentsList.begin();
         ...
         ++itr)
    {
        if (itr->intersects(_currentLevel, _min, _max))
            return true;
    }
